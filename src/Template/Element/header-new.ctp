...
echo $this->Html->link($myImageVar, [
  ...
  //pr ($this->request->params); 
   ...
   // pr($this->request->query);
     ...
     $urlControllersArray = array('users', 'sun-signs', 'products', 'elite-users', 'orders','mini-blogs');
          ...
          $action_dk = $translator->translate($action);
           ...
           if(isset($current_product_id) && !empty($current_product_id) && $language == 'dk')
            ...
            if($action == 'astrology') {
                ...
                $action_dk = $translator->translate($action).'/'.$slug_dk;
                $action = $action.'/'.$this->Custom->getEnglishTranslationOfProduct($this->request->params['pass'][0]);
            } else {
    ...
    //if(strpos($action, 'thank-you') != false && !empty($this->request->query))
     if((strpos($action, 'thank-you') != false && !empty($this->request->query)) || (strpos($action, 'thanks') != false && !empty($this->request->query)) )
    {
    ...
    elseif( in_array($controller, $urlControllersArray) && $this->request->params['action'] != 'astrology') 
    {
      ...
      if( $_SERVER['REMOTE_ADDR'] == '103.254.97.14' || $_SERVER['REMOTE_ADDR'] == '103.248.117.12' ) {
        if( ($controller == 'products') && (isset($this->request->params['pass'][0]) ) && (!empty($this->request->params['pass'][0]) ) ) {
          $en_url = $en_url;
          $dk_url = $dk_url;
        }
     } else {
...
?> 
      ...
      <ul>
        ...
        <?php else : ?>
          ...
          <li> <?php echo $this->Html->link(__('Log In'),['controller'=>'Users','action'=>'login'])?> </li>
          <li>
