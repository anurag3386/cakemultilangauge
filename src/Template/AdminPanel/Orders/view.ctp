    ...
    <div class="box">
      ...
      <?php
          ...
          {//changed by anurag dubey Nov 13 2017
            ...
            if(strtolower($orderDetail['currency']['symbol'])=='kr.'){
                $price = round($orderDetail['price'],0);
            } else {
                $price = sprintf('%0.2f', $orderDetail['price']);
            }
               ...
               <div class = "panel-body">
                  <table class="table table-hover">
                   <tbody>
                    ...
                    <tr>
                       <td>Price</td>
                       <td><?php echo $orderDetail['currency']['symbol'].$price?></td>
                    </tr>
                     <tr>
                       <td>User Name</td>
                       <td>
                       <?php 
                       if($orderDetail['another_person'] == 0)
                       {
                         if($orderDetail['user_id'] == 0)
                         {
                           echo $orderDetail['guest_user_product_detail']['first_name']. " " . $orderDetail['guest_user_product_detail']['last_name'];
                         }
                         else
                         {
                           echo ucwords($orderDetail['profile']['first_name']." ".$orderDetail['profile']['last_name']);
                         }

                       }
                       else
                       {
                          $data = $this->Custom->getAnotherPersonDetail($orderDetail['user_id']);
                          if($data)
                          {
                            echo  $this->Custom->getFullName($data['profiles']['first_name'], $data['profiles']['last_name'])." <br>[".$this->Custom->getFullName( $data['fname'], $data['lname'])."]";
                          }
                       }
                     ...
                     <?php 
                          if(!empty($orderDetail['birthdata']))
                          {
                     ?>
                     <tr>
                     ...
                     <tr>
                      ...
                      <?php
                       //if(strtolower($orderDetail['state']['name']) == 'closed' && file_exists(WWW_ROOT ."reports/var/spool/".$orderDetail['id'].".bundle.pdf")):
                       
                       if( file_exists(WWW_ROOT ."reports/var/spool/".$orderDetail['id'].".bundle.pdf") && $orderDetail['product']['category_id'] == 2):
                        ?>
                      <tr>
                        ...
                        <tr>
                         ...
                         <td>
                          <div class="row">
                             <div class="col-md-3">
                               <?php 
                                echo $this->Form->select('status', $statusOptions, ['class' => 'form-control col-md-3', 'style' => ['text-transform: capitalize'], 'default' => $orderDetail['state']['id'], 'id' => 'status'] ); 
