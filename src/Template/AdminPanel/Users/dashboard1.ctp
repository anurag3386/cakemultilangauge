...
<?php
  //if ($_SERVER['REMOTE_ADDR'] == '103.248.117.12' || $_SERVER['REMOTE_ADDR'] == '103.254.97.14') {
?>
...
<div class="row">
    <div class="col-md-3">
      <?php
        $dropDownOptions = [1 => 'Horoscope Calendar Subscribers', 2 => 'Soft Exit Users'/*, 3 => 'Both'*/];
  ...
  <div class="col-md-12">
    ...
    <ul class="chart-options">
      <li id="cal-subs-daily" class="active">Daily</li>
...
<script type="text/javascript">
  ...
  $('#chartoption').on('change', function(){
    ...
    $("li#cal-subs-daily").addClass("active");
    drawChart(1);
  });
  ...
  function drawChart(timePeriod='') {
    ...
    if(timePeriod!='' && timePeriod != undefined) {
      ...
      if (/*adminSelection==1 && */timePeriod==2) {
        hAxisTitle = hAxisTitle+"\n <?= $weekDataDes; ?>";
      }

      if (/*adminSelection==1 && */timePeriod==3) {
    ...
    $.ajax({
      ...
      success: function (data1) {
        ...
        var options = {
                  pieSliceText: 'label',
                  height: 400,
                  title: 'Horoscope Calendar Subscribers',
                  hAxis: {
                    title: hAxisTitle
                  },
                  vAxis: {
...
$('#cal-subs-weekly').on('click', function() {
  $(".chart-options li").removeClass("active");
  $("li#cal-subs-weekly").addClass("active");
  drawChart(2);
});

$('#cal-subs-monthly').on('click', function() {
  $(".chart-options li").removeClass("active");
  $("li#cal-subs-monthly").addClass("active");
  drawChart(3);
});

$('#cal-subs-yearly').on('click', function() {
  $(".chart-options li").removeClass("active");
  $("li#cal-subs-yearly").addClass("active");
  drawChart(4);
});
