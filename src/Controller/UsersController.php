...
class UsersController extends AppController {
    ...
    /*
     * Simple captcha code on signup process
     * Created By : Kingslay
    ...
    function getCaptcha () {
      ...
      $code=rand(1000,9999);
      $this->request->session()->write('code', $code);
      $im = imagecreatetruecolor(50, 24);
      //$bg = imagecolorallocate($im, 22, 86, 165); //background color blue
      $bg = imagecolorallocate($im, 228, 22, 91); //background color according to our theme color
    ...
    public function signUp() {
        ...
        elseif ($this->request->is('post')) :
                /*
                 * Validate user from robots on signup
                 * Created By : Kingslay
                 * Created Date : May 03, 2017
                 */
              //if( $_SERVER['REMOTE_ADDR'] == '103.254.97.14' || $_SERVER['REMOTE_ADDR'] == '103.248.117.12' ) {
                if(isset($this->request->data["captcha"]) && ($this->request->data["captcha"]!="") && ($this->request->session()->read('code') == $this->request->data["captcha"])) {
                  $this->request->session()->delete('code');
                } else {
                  $this->request->session()->write('signUpUserDetails', $this->request->data);
                  $this->Flash->error(__('Please submit captcha code properly'));
                  return $this->redirect([ 'controller' => 'users', 'action' => 'sign-up']);
                }
                /*
                 * Allow a valid email id if anyone(user) has been remove JS validations in frontend
              ...
              if($find->isEmpty()):
                  ...
                  if ($result = $this->Users->save($data)) {
                      ...
                      if($this->request->session()->read('locale') == 'da') {
                        $welcomeMailSubject = 'Velkommen til at Astrowow.com';
                      } else {
                        $welcomeMailSubject = 'Welcome to Astrowow.com‏';
                      }
                      $data = [
    ...
    protected function userStatus ($data) {
      ...
      /*if($this->checkIp()){
        echo Security::hash(trim($data['password'])); pr($check); die('kkkkkkkkkkk');
      }*/
      if ($check) {
     ...
     * Created Date : March 28, 2017
     */
    protected function changePopupUserStatus ($id) {
      $this->loadModel('IpAddresses');
      $check = $this->IpAddresses->find()->where(['user_id' => $id, 'created <=' => time(), 'end_date >=' => time()])->first();
      if ($check) {
          
          $query1 = $this->IpAddresses->query();
          $updateData1 = $query1->update()
                        ->set(['log_status' => '1'])
                        ->where(['user_id' => $id])
                        ->execute();
    ...
    public function login($userId = null) { // If $userId exists that means admin wants to login with that account
        ...
        if ( $this->request->is('post') || !empty ($userId) ) {
            ...
            if (!empty ($userId)) {
              ...
              if (empty($user)) {
                $this->Flash->error(__("Invalid User"), ['escape' => false]);
              ...
              /*if($this->checkIp()){
                ...
                $hasher = new DefaultPasswordHasher();
                echo $password = $hasher->hash(trim($this->request->data['password']));die;
              }*/
              $checkUserDetail = $this->userStatus($this->request->data()); //Set 
              $user = $this->Auth->identify();
              /*if($this->checkIp()){
                $this->request->session()->write('profileUpdated', $user['id']);
            ...
            if ($user) {
                //changed by anurag dubey 16 nov 2017
                //if ($this->checkIp()) {
                    if($user['role']=='elite'){
                ...
                //if($this->checkIp()) {
                  if($user['popup_user'] == 1) {
                    $this->changePopupUserStatus($user['id']);
                  }
                //}
                ...
                //if( $_SERVER['REMOTE_ADDR'] == '103.254.97.14' || $_SERVER['REMOTE_ADDR'] == '103.248.117.12' ) {
                	if(empty($userId) && $this->request->data['remember_me']) {
                		$date = date('Y-m-d', strtotime('+1 month'));
                		$userdata_id = $user['id'];
                		$userdata_username = $user['username'];
	                    setcookie("userDetail_id", base64_encode($userdata_id), strtotime($date), "/", "",  0);
	                    setcookie("userDetail_username", $userdata_username, strtotime($date), "/", "",  0);
                  	}
                //}
                ...
                // }
                  if( $this->request->session()->read('user_id') && $user['step'] == 2 && $this->request->session()->read('Order')) {
                  ...
                  } elseif (!empty($session->read('support-ticket-url'))) { //Support ticket url
                    return $this->redirect($session->read('support-ticket-url'));
                  } else {
                    ...
                    if (strtolower($user['role']) == 'elite') {
                      ...
                      if (!empty($checkEliteMemberValidity)) {
                        // Setting cookie for blog page
                      setcookie("elite", $data['id'], time()+3600, "/", "",  0);
                       // valid elite member
                ...
                if($this->Users->save($user)){
                	...
                	$user = $this->Users->find('all')
                  	...
                  	if(strtolower($this->request->session()->read('locale') ) == 'da' ) {
                     	$base = Router::url('/dk/brugere/nulstille-adgangskode-token/', true);
                  	} else {
                    	$base = Router::url('/users/reset-password-token/', true);
                  	}
                  	$link = $base.$user['reset_password_token'];
                  	$link = "<a href='".$link."'>".$link."</a>";
                  	$arr['send_to'] = $user['username'];
                  	$arr['action'] = 'password reset request';
                  	$arr['subject'] = __('Password Reset Request - DO NOT REPLY');
                  	$name = $this->getFullName( $user['profile']['first_name'] , $user['profile']['last_name'] );
                  	$this->loadModel('EmailTemplates');
                  	$template = $this->EmailTemplates->find('translations')
                                                  ->where(['EmailTemplates.short_code' => 'forgot_password'])
                  	...
                  	$thank_you = $ThankyouMailOtherActionsTable->patchEntity($ThankyouMailOtherActionsEntity, $arr);
                  
                  	if ($ThankyouMailOtherActionsTable->save($thank_you)) {
  ...
  function resetPasswordToken($reset_password_token = null) {
            ...
            if ($this->Users->save($user)) {
                ...
                //if($this->checkIp()) {
                  ...
                  $user  = $this->Users->find('all')
                                 ...
                                 ->first();
                  $arr['send_to'] = $user['username'];
                  $arr['action'] = 'password reset successfully thank you mail';
                  $arr['subject'] = __('Password Reset Request - DO NOT REPLY');
                  $name = $this->getFullName( $user['profile']['first_name'] , $user['profile']['last_name'] );
                  $this->loadModel('EmailTemplates');
                  $template = $this->EmailTemplates->find('translations')
                                                  ->where(['EmailTemplates.short_code' => 'password_changed'])
                                                  ->select(['EmailTemplates.content', 'variables'])
                                                  ->first();
                  $body = html_entity_decode($template['content']);
                  $body = str_replace('{NAME}', $name, $body);
   ...
   protected  function __validToken($token_created_at) {
    ...
    public function logout(){
        ...
        //if( $_SERVER['REMOTE_ADDR'] == '103.254.97.14' || $_SERVER['REMOTE_ADDR'] == '103.248.117.12' ) {
          if(!empty($_COOKIE['userDetail_id']) && !empty($_COOKIE['userDetail_username'])) {
            unset($_COOKIE['userDetail_id']);
            setcookie('userDetail_id', '', time() - 3600, '/'); // empty value and old timestamp
            unset($_COOKIE['userDetail_username']);
            setcookie('userDetail_username', '', time() - 3600, '/'); // empty value and old timestamp
          }
        //}
    ...
    public function dashboard () {
        ...
        if ($loggedInUserId) {
            ...
            **/
            ...
            //if($this->checkIp()) {
              if($userDetails['popup_user'] || $userDetails['mobile_app_user']) { // If user is signup using sun-sign detail paeg popup
                $userProfileDetails = $this->Profiles->find()->where(['user_id' => $loggedInUserId])->first();
                if (empty($userProfileDetails['last_name']) || empty($userProfileDetails['gender'])) {
                  if($userDetails['mobile_app_user']) {
                    $this->Flash->error(__('Please fill first name, last name and gender.'));
                  } else {
                    $this->Flash->error(__('Please fill last name and gender.'));
                  }
            ...
            //$latestPosts = $this->MiniBlogs->find ()->where(['MiniBlogs.status' => 1])->order(['MiniBlogs.sort_order' => 'ASC'])->limit(3)->toArray();
            ...
            if (!empty($anotherPersonDetail)) { // If another person is selected
                ...
                /*if( $_SERVER['REMOTE_ADDR'] == '103.254.97.14' || $_SERVER['REMOTE_ADDR'] == '103.248.117.12' ) {
                  echo '<pre>'; print_r($subscriptionActivateOrNot); die;
                }*/
     ...
     public function dashboardPm () {
        ...
        if ($loggedInUserId) {
            /** Checking registered user
              Created By: Stan Field 24th Nov 2016
            **/
             $userDetails = $this->Users->get($loggedInUserId);
             $step = $userDetails['step'];
             if($step == 1) {
            ...
            /*if (!empty($subscriptionActivateOrNot)) {
              $session->write('calendar_start_date', $subscriptionActivateOrNot['start_date']);
              $session->write('calendar_end_date', $subscriptionActivateOrNot['end_date']);
            }*/
            $this->set(compact('perchasedReport', 'latestPosts'));
            //$this->render('dashboard');
        } else {
        ...
        if ($logeedInUser) {
            ...
            if (!empty($this->request->data)) {
                ...
                if ( isset($this->request->data['btnUpdateBirthDetail']) && !empty($this->request->data['btnUpdateBirthDetail'])) {
                    ...
                    $userGenderQuery = $this->Profiles->query();
                    $updateUserGender = $userGenderQuery->update()
                            ->set(['gender' => $this->request->data['rdoGender']])
                    ...
                    if (is_numeric($user_id) || !empty($myaccount)) { // For loggedin user
                        ...
                        if ($this->BirthDetails->save($data)) {
                          ...
                          /*
                           * Update data for birthday report in birthdata table
                           * Created By : Kingslay
                           * Created Date : April 21, 207
                           */
                          //if( $_SERVER['REMOTE_ADDR'] == '103.254.97.14' || $_SERVER['REMOTE_ADDR'] == '103.248.117.12' ) {
                            $BirthdayReportOrder_id = $this->getOrderIdForBirthdayReport($user_id);
                            $this->loadModel('Cities');
                            $cityname = $this->Cities->find()->where(['id' => $city])->first();
                            $countryShortCode = $this->tableData('Countries', $country, 'abbr');
                            $this->loadModel ('Birthdata');
                            $query = $this->Birthdata->query();
                            $updateData = $query->update()
                                  ->set(['day' => $manipulatedDate[0], 'month' => $manipulatedDate[1], 'year' => $manipulatedDate[2], 'hour' => $this->request->data['hours'], 'minute' => $this->request->data['minutes'], 'zoneref' => $zone, 'summerref' => $type, 'place' => $cityname['city'], 'state' => $countryShortCode['abbr'], 'longitude' => $cityname['longitude'], 'latitude' => $cityname['latitude'], /*'gender' => $this->request->data['rdoGender'],*/ 'modified' => date('Y-m-d H:i:s')])
                                  ->where(['order_id' => $BirthdayReportOrder_id])
                                  ->execute();
                            //if($this->checkIp()) {
                              $this->loadModel ('Users');
                              $uquery = $this->Users->query();
                              $user_updateData = $uquery->update()
                                  ->set(['profile_updated' => 1, 'modified' => date('Y-m-d H:i:s')])
                                  ->where(['id' => $user_id])
                                  ->execute();
                            //}
                    ...
                    if (is_numeric($user_id) || !empty($myaccount)) { // For loggedin user
                        ...
                        if(!empty($profile_id)) {
                          ...
                          $profilesEntity = $this->Profiles->patchEntity($profilesEntity, $mydata, ['translations' => true]);
                          if ($this->Profiles->save($profilesEntity)) {
                            $profileUpdateState = true;
                          }
                        } else { // for mobile app users
                          $pInsertQuery = $this->Profiles->query();
                          //$pquery = $pInsertQuery->query();

                          $pInsertQuery->insert(['user_id', 'gender', 'first_name', 'last_name', 'language_id'])
                              ->values([
                                  'user_id' => $user_id,
                                  'gender' => $mydata['gender'],
                                  'first_name' => $mydata['first_name'],
                                  'last_name' => $mydata['last_name'],
                                  'language_id' => ($session->read('locale') == 'dk' || $session->read('locale') == 'da') ? 2 : 1
                              ])
                              ->execute();
                          if($pInsertQuery) {
                            $profileUpdateState = true;
                        ...
                        if ($profileUpdateState) {
                          ...
                          //if( $_SERVER['REMOTE_ADDR'] == '103.254.97.14' || $_SERVER['REMOTE_ADDR'] == '103.248.117.12' ) {
                            ...
                            $query = $this->Birthdata->query();
                            $updateData = $query->update()
                                  ->set(['first_name' => $mydata['first_name'], 'last_name' => $mydata['last_name'], 'name_on_report' => $mydata['first_name'].' '.$mydata['last_name'], 'modified' => date('Y-m-d H:i:s')])
                                  ->where(['order_id' => $BirthdayReportOrder_id])
                                  ->execute();
                            //if($this->checkIp()) {
                              $this->loadModel ('Users');
	            ...
	            if (is_numeric($user_id)) { // For registered User
	                $userProfile = $this->Profiles->find()->select(['id', 'user_id', 'first_name', 'last_name', 'gender'])->where(['user_id' => $user_id])->first();
    ...
    function deleteAnotherPerson ($user_id) {
      ...
      if ($loggedInUserId) {
        ...
        if ($updateData) {
          ...
          $session->write('selectedUser', $session->read('Auth.User.id'));
    ...
    function deletepermanent () {
        ...
        if ($logeedInUser) {
            $this->loadModel('IpAddresses');
            $popup_userExistancy = $this->IpAddresses->find()->where(['user_id' => $logeedInUser])->first();
            if(!empty($popup_userExistancy)) {
              $IpAddressesEntity = $this->IpAddresses->get($popup_userExistancy['id']);
              $this->IpAddresses->delete($IpAddressesEntity);
            }
            ...
            if ($this->request->is('post') && !empty($this->request->data)) {
              if (empty($this->request->data['txtFirstName']) || empty($this->request->data['txtLastName']) || empty($this->request->data['rdoGender']) || empty($this->request->data['user_dob_edit']) /*|| empty($this->request->data['birthhour']) || empty($this->request->data['birthminute'])*/ || empty($this->request->data['ddBirthCountry']) || empty($this->request->data['ddBirthCity']) ) {
                ...
                if ($anotherPersonsTable->save($anotherPerson)) {
                    ...
                    $this->request->session()->write ('selectedUser', 'anotherPerson_'.$lastid);
    ...
    public function thankYou () {
      ...
      $this->set(compact('meta', 'canonical'));
    	...
    	if( $this->request->is('get') && !empty($this->request->query('hash'))) {
            ...
            //if($this->checkIp()) {
              $this->loadModel('Orders');
              $checkPayerIdExistancy = $this->Orders->find()->where(['payer_order_id' => $session->read('subscriptionCalendarOrderId')])->count();
              if ($checkPayerIdExistancy > 0) {
                $session->delete('subscriptionCalendarOrderId');
                return $this->redirect(['controller' => 'users', 'action' => 'dashboard']);
              }
            //}
          	$subcribesTable = $this->Orders; //TableRegistry::get('Orders');
          	$entity = $subcribesTable->newEntity();
            ...
            if (!empty($checkSubscriptionStatus)) {
          	...
          	$data = $subcribesTable->patchEntity($entity, $mydata, ['associated' => ['OrderTransactions', 'Subscribes']] );

            /*if($this->checkIp()) {
              pr($data['product_type']); die;
            }*/
          	...
          	if ($subcribesTable->save($data)) {
                ...
                //if($this->checkIp()) {
                  ...
                  $thankyouMailEntity['product_type'] = $data['product_type'];
                ...
                /*} else {
                  ...
                  $this->request->session()->write('purchaseEvent.Price', $mydata['order_transaction']['amount']);
                  $this->request->session()->write('purchaseEvent.Currency', $mydata['order_transaction']['currency_code']);
                  //if( $_SERVER['REMOTE_ADDR'] == '103.254.97.14' || $_SERVER['REMOTE_ADDR'] == '103.248.117.12' ) {
                    //echo '<pre>'; print_r($this->request->session()->read()); die;
                    $msgBody = $this->calendarAndEliteSubscriptionAdminMail ($data->id, $this->request->query('txnid'));
                    $emailTemplate = new EmailTemplatesController();
                    $emailTemplate->sendMailToAdmin('Calendar subscription order received', $msgBody);
                  //}
                }*/
     ...
     */
    /*function calendarAndEliteSubscriptionAdminMail ($order_id, $transaction_id='') {
      $this->loadModel('Orders');
      $this->loadModel('Products');
      $this->loadModel('Currencies');

      //if($this->checkIp()){
        //astrowow.com/users/calendar-and-elite-subscription-admin-mail/503207
        $orderDetail = $this->Orders->find()
                        ...
                        ->select([ 'Orders.id', 'Orders.product_id', 'Orders.price', 'Orders.email', 'Orders.payer_order_id', 'Orders.user_id', 'Orders.product_type', 'Orders.another_person', 'Orders.currency_id', 'Orders.order_date', 'products.id', 'products.name', 'currencies.id', 'currencies.symbol'])
                        ->where(['Orders.id' => $order_id])
                        ->first();
        $userdDataDetailsName = '';
        if($orderDetail['another_person']){
          $this->loadModel('AnotherPersons');
          $userdDataDetails = $this->AnotherPersons->find()
                                                  ->join([
                                                      'Profiles' => [
                                                          'table' => 'profiles',
                                                          'type' => 'INNER',
                                                  ...
                                                  ->select(['AnotherPersons.added_by', 'AnotherPersons.fname', 'AnotherPersons.lname', 'Profiles.user_id', 'Profiles.first_name', 'Profiles.last_name'])
                                                  ->where(['AnotherPersons.id' => $orderDetail['user_id']])
                                                  ->first();


          $userdDataDetailsName = ucwords($userdDataDetails['Profiles']['first_name'].' '.$userdDataDetails['Profiles']['last_name'].' ['.$userdDataDetails['fname'].' '.$userdDataDetails['lname'].']');
        } else {
          $this->loadModel('Profiles');
          $userdDataDetails = $this->Profiles->findByUserId($orderDetail['user_id'])->first();
          $userdDataDetailsName = ucwords($userdDataDetails['first_name'].' '.$userdDataDetails['last_name']);
        }
      // } else {
      //   $orderDetail = $this->Orders->find()
      //                   ->join([
      ...
      //                   ->first();
      // }
      if ($orderDetail['product_id'] == 53) {
        $productname = 'Calendar Subscription';
      } else {
        $productname = 'Elite Membership Subscription';
      }
      ...
      return $template = '<table>
                          <tr><td>Hi,</td></tr>
                          <tr><td>&nbsp;</td></tr>
                          <tr><td>You have received a '.$productname.' order. Order details are mentioned below: </td></tr>
                      </table>
                      <table style="border: 1px solid #e4165b; width: 800px;">
                        <tbody>
                          <tr>
                            <td>
                              <h3 style="color: #fff; text-align: center; background-color: #e4165b; border-color: #337ab7;" class="panel-title">'.$productname.' order details</h3>
                            </td>
                          </tr>
                          <tr>
                            <td>
                              <table>
                                ...
                                <tr>
                                  <td>Order Date : </td>
                                  <td>'.date('d-m-Y h:i:sa', strtotime($orderDetail['order_date'])).'</td>
                                </tr>
                                <tr>
                                  <td>Order No. : </td>
                                ...
                                <tr>
                                  <td>Amount : </td>
                                  <td>'.$orderDetail["currencies"]["symbol"].$orderDetail["price"].'</td>
                                </tr>
                                <tr>
     ...
     * Modified Date : Nov. 22, 2016
     */
    function wheelKk ($apikey, $uid, $task, $aper='') {
      $this->autoRender = false;
      $session = $this->request->session();
      $user_id = !empty($session->read('Auth.User.id')) ? $session->read('Auth.User.id') : $session->read('user_id');
        if ($user_id) {
          ...
          if ($forwardStatus) {
              require_once ( ROOT . DS . 'vendor' . DS  . 'Horoscope-wheel' . DS . 'personal-wheel_kk.php');
          }
    ...
    function mycalendarIcons () {
      ...
      //echo 'API => '.$apikey.' => Date => '.$date.' => User => '.$user_id.' => Language => '.$lang; die;
      $this->autoRender = false;
      $dataf = require_once ( ROOT . DS . 'vendor' . DS  . 'calendar-icons' . DS . 'class.daily.personal.calendar.icons_pm.php');
    ...
    function subscriptionCalendar () {
      ...
      if ($user_id) {
          $this->autoRender = false;
          /*if( $_SERVER['REMOTE_ADDR'] == '103.254.97.14' || $_SERVER['REMOTE_ADDR'] == '103.248.117.12' ) {
            //require_once ( ROOT . DS . 'vendor' . DS  . 'calendar-icons' . DS . 'personal-daily-horoscope_pm.php');
            require_once ( ROOT . DS . 'vendor' . DS  . 'calendar-icons' . DS . 'personal-daily-horoscope_kk_latest.php');
          } else {*/
    ...
    /**
     * To show subscription calendar data on clicking next or previous arrows
     * created By : Kingslay <kingslay@123789.org>
     * Created Date : April 26, 2017
     */
    function subscriptionCalendarNextMonth ($monthNo) {
      $apikey = md5('astrowow.com');
      $date = date("m-d-Y");
      $session = $this->request->session();
      $user_id = !empty($session->read('Auth.User.id')) ? $session->read('Auth.User.id') : $session->read('user_id');
      ...
      $lang = !empty($this->request->session()->read('locale')) ? $this->request->session()->read('locale') : 'en';
      if ($user_id) {
          $this->autoRender = false;
          /*if( $_SERVER['REMOTE_ADDR'] == '103.254.97.14' || $_SERVER['REMOTE_ADDR'] == '103.248.117.12' ) {
            //require_once ( ROOT . DS . 'vendor' . DS  . 'calendar-icons' . DS . 'personal-daily-horoscope_pm.php');
    ...
    /**
     * To show subscription calendar data for non-subscribers only for current month
     * created By : Kingslay <kingslay@123789.org>
     * Created Date : April 28, 2017
     */
    function subscriptionCalendarForNonSubscrideUsers ($monthNo='') {
      $apikey = md5('astrowow.com');
      $date = date("m-d-Y");
      $session = $this->request->session();
      $user_id = !empty($session->read('Auth.User.id')) ? $session->read('Auth.User.id') : $session->read('user_id');
      ...
      if ($user_id) {
          $this->autoRender = false;
          //if( $_SERVER['REMOTE_ADDR'] == '103.254.97.14' || $_SERVER['REMOTE_ADDR'] == '103.248.117.12' ) {
            require_once ( ROOT . DS . 'vendor' . DS  . 'calendar-icons' . DS . 'personal-daily-horoscope-non-subscribe.php');
          /*} else {
            require_once ( ROOT . DS . 'vendor' . DS  . 'calendar-icons' . DS . 'personal-daily-horoscope.php');
          }*/
      } else {
    ...
    function subscriptionCalendarPm () {
      $apikey = md5('astrowow.com');
      $date = date("m-d-Y");
      $session = $this->request->session();
      $user_id = !empty($session->read('Auth.User.id')) ? $session->read('Auth.User.id') : $session->read('user_id');
    ...
    function horoscopeCalendarInRange($startDateInMS, $endDateInMS, $monthNo=null) {
      $date = date("m-d-Y", $startDateInMS);
      $session = $this->request->session();
      $user_id = !empty($session->read('Auth.User.id')) ? $session->read('Auth.User.id') : $session->read('user_id');
      $user_id = !empty($session->read('selectedUser')) ? $session->read('selectedUser') : $user_id;
      $lang = !empty($this->request->session()->read('locale')) ? $this->request->session()->read('locale') : 'en';
      if ($user_id) {
          $this->autoRender = false;
          require_once ( ROOT . DS . 'vendor' . DS  . 'calendar-icons' . DS . 'personal-daily-horoscope-range.php');
      } else {
          $this->Flash->set(__("You can't access that location."), [ 'element' => 'error' ]);
          $this->redirect(['controller' => 'users', 'action' => 'login']);
      }
    ...
    /**
     * This function used to user authentication for mobile app
     * created By : Kingslay <kingslay@123789.org>
     * Created Date : August. 30, 2017
     */
    function mobileUserLogin ($email, $password) {
      $this->autoRender = false;
      $this->request->data['username'] = trim($email);
      $this->request->data['password'] = trim($password);
      if(!empty($this->request->data['username']) && !empty($this->request->data['password'])) {
        ...
        if(!empty($user)){
          ...
          echo json_encode('unauthorized user.'); die;
        }
      } else {
        echo json_encode('unauthorized user.'); die;
      }
     ...
     * Created Date : August. 30, 2017
     */
    function mobileUserSignup ($email, $password, $language, $portal_id) {
      $this->autoRender = false;
      $returnData = [];
      $checkUserExistancy = $this->Users->find()->select(['id'])->where(['Users.username' => $email])->first();
      if(!empty($checkUserExistancy) && (count($checkUserExistancy) > 0)) {
        $returnData['UserId'] = $checkUserExistancy['id'];
      } else {
        $this->request->data['username'] = $email;
        $this->request->data['password'] = $password;
        $this->request->data['mobile_app_user'] = 1;
        $this->request->data['step'] = 1;
        $this->request->data['preview_report'] = '';
        $this->request->data['popup_user'] = 0;
        $entity = $this->Users->newEntity();
     ...
     * Created Date : August. 30, 2017
     */
    function validateMobileUserWithEmailId ($email) {
      $this->autoRender = false;
      $checkUserExistancy = $this->Users->find()->select(['id'])->where(['Users.username' => $email])->first();
      if(!empty($checkUserExistancy) && (count($checkUserExistancy) > 0)) {
        echo json_encode($checkUserExistancy); die;
      } else {
        echo json_encode('unauthorized user.'); die;
      }
    }
    ...
    function sendMobileUserForgotPasswordLink($userid) {
      ...
      if($this->Users->save($user)){
        ...
        $thank_you = $ThankyouMailOtherActionsTable->patchEntity($ThankyouMailOtherActionsEntity, $arr);
        if ($ThankyouMailOtherActionsTable->save($thank_you)) {
          echo json_encode('successfully sent.');
        }
      } else {
    ...
    /*
     * This function is used to create xml file of horoscope wheel
     * Created By : Kingslay@123789.org
     * Created Date : Sep. 19, 2017 
     * @params $apikey varchar
     * @params $lang varchar
     ...
     * @params $hour int
     * @params $minute int
     * @params $place varchar
     * @params $country varchar
     * @params $fname varchar
     * @params $lname varchar
     * @params $gender varchar
    */
    function wheelXml ($apikey, $lang='en', $email, $day, $month, $year, $hour, $minute, $place, $country, $fname, $lname, $gender='M') {
      $this->autoRender = false;
      require_once ( ROOT . DS . 'vendor' . DS  . 'Horoscope-wheel' . DS . 'wheel-xml.php');
    }
    
    function showxml ($filename) {
      $this->autoRender = false;
      $this->redirect('https://www.astrowow.com/Horoscope-wheel-xml/'.$filename);
    }
