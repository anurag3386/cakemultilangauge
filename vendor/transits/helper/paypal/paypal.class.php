...
class paypal_class {
    ...
    function paypal_class() {
        ...
        $this->ipn_log = true;
        $this->ipn_response = '';
        //$this->admin_email = 'dhruv.sarvaiya@n-techcorporate.com';
        //$this->admin_email = 'parmaramit1111@gmail.com';
...
//        else if (eregi("RECEIVED",$this->ipn_response)) {
        else if (preg_match("/RECEIVED/",$this->ipn_response)) {
            // Received Payment
            $this->log_ipn_results(true);
            return true;
        }
//        else if (eregi("INVALID",$this->ipn_response)) {
        else if (preg_match("/INVALID/",$this->ipn_response)) {
            // Invalid IPN transaction.  Check the log for details.
            $this->last_error = 'IPN Validation Failed.';
            $subject = 'AMIT == Paypal IPN Gateway - '. $this->last_error . " = " . $this->ipn_response;
            $to = $this->admin_email;
            $body =  "An instant payment notification was failed\n";
    ...
    function log_ipn_results($success) {
        ...
        if ($success) {
            ...
            $body .= 'FAIL: '.$this->last_error."\n";
        }

        // Log the POST variables
        $text .= "IPN POST Vars from Paypal:\n";
        $body .=  "IPN POST Vars from Paypal:\n";
        foreach ($this->ipn_data as $key=>$value) {
            $text .= "$key=$value, ";
            $body .= "\n$key: $value";
        }
