...
// 		echo 'Amount  = ' . number_format(floatval(str_replace(',', '', $product_price)) ,2) . "<br />";
// 		echo 'Amount = ' . $product_price . "<br />";
		...
		if ($this->validate_ipn()) {
			...
			try {
				...
				if( $order_state == 2 ||  $order_state == 12) {
					...
					if($objOrder->SendPaymentAcceptMailForReport($order_id, $languageid)) {
