<?php
/**
 * Script: /helper/paypal/elite/class.payment.paypal.wow.elite.php
 * Author: Amit Parmar
 *
 * Description
...
require_once ( 'class.payment.paypal.generic.elite.php');
require_once ( 'class.payment.paypal.wow.elite.php');

define('ORDER_URL', ROOTPATH.'bal/order.php');
define('USER_URL', ROOTPATH.'bal/user.php');

require_once(ROOTPATH."bal/elite_user.php");
require_once(ROOTPATH."bal/class.elite.product.php");
require_once(ROOTPATH."dal/elite-userRepository.php");
...
if (isset ( $action )) {
	...
	$transaction = new eLitePaypalTransaction ();
	switch ($action) {
		case 'process' :
			$orderId = $_REQUEST ['orderid'];
			$currency_code = $_REQUEST ['currency_code'];
			$product_item_number = $_REQUEST ['prefix'];
			...
			break;
		case 'success' :
			$transaction->manage_success ( SUCCESS_URL_RPT_ELITE_PAYMENT);
			break;
		case 'cancel' :
