    ...
    function manage_ipn() {
        ...
        if ($this->validate_ipn()) {
            ...
            if( $order_state == 2) {
				...
				mail('parmaramit1111@gmail.com', "OrderNo:".$order_id."-".$order_state, print_r($this->ipn_data));
				//mail('dhruvraj.sarvaiya@gmail.com', "userId:".$user_id."-".$order_state, print_r($this->ipn_data));

				// order status 12 = ready
                $returnValue = $objOrder->UpdateOrderStatusFromIPN($order_id, 12);
				
                $result = $objUser->GetUserDetailForMail($user_id);
                if(count($result)>0) {
                    $email = $result[0]['UserName'];
                    $first_name = $result[0]['FirstName'];
                    $last_name = $result[0]['LastName'];
					
					$sender = "parmaramit1111@gmail.com";
					$to = $email;
					...
					// send mail to payment mail
                	mail('paymentorder@astrowow.com', $subject, $body);
                }
