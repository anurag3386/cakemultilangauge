...
class AstrologCalculator {
    ...
    function callAstrolog() {
        ...
        $this->m_cache = $this->getAstrologData($this->m_args);
    }

    function getAstrologData($getData) {
        $getVar         = urlencode($getData." -YQ 0");
        //$getOutput      = file_get_contents("http://54.153.95.173/astro.php?q=".$getVar); //file_get_contents("http://54.67.50.240/astro.php?q=".$getVar);
        $getOutput      = file_get_contents("http://52.52.17.200/astro.php?q=".$getVar); //file_get_contents("http://54.67.50.240/astro.php?q=".$getVar);
        return $getOutput;
    }
