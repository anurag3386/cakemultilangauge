...
*/
$orderDAL = new Order();
//$orderQueueList = $order->GetList ( array(
...
//                array ('product_item_id', '=', "19"),                                     /* Personal/full/pc3 */
//            ) );
...
//        $reportOptionObj = $reportOptionDAL->GetList(array(array('orderId', '=', $orderitem->order_id)));
//        foreach ($reportOptionObj as $key => $rptOptItem) {
//            $reportLanguage = $rptOptItem->language;
//            $memberName     = $rptOptItem->name;
//            $startDate      = $bData->start_date;
//            $reportDuration = $rptOptItem->duration;
//            $emailAddressId = $rptOptItem->emailaddressid;
//            $reportPageSize = strtoupper($rptOptItem->paper_size);
//        }
...
function SetBirthInformation($birthentry, &$wheel) {
    ...
    $isTimedData = ( (strtoupper(trim($birthentry->untimed)) == 'N') ? true : false );
...
//    $fullbirthplace = preg_split("/(>)/i",$acsatlas->placename);
