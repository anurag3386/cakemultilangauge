<?php
include("wheel-constants.php");

$retrograde = safeEscapeString(urldecode($_GET["rx1"]));

if (get_magic_quotes_gpc()) {
    $longitude = unserialize(stripslashes(urldecode($_GET["p1"])));
    $hc = unserialize(stripslashes(urldecode($_GET["hc1"])));
} else {
    $longitude = unserialize(urldecode($_GET["p1"]));
    $hc = unserialize(urldecode($_GET["hc1"]));
}
...
//$white = imagecolorallocate($im, 0, 132, 255);
...
for ($i = 1; $i <= 12; $i = $i + 1) {
    ...
    if ($i < 12) {
        $angle = ($i * 30.0);
        $c0 = imagecolorallocate($im, $sign_colour[$i][0], $sign_colour[$i][1], $sign_colour[$i][2]);
        $end = fmod(($angle + 30.0), 360.0);

        //imagefilledarc($im, $x1, $y1, $x2 , $y2 , $angle, $degrees, $c0, IMG_ARC_PIE);
        //imagefilledarc($im, $startX, $y1, $diameter, 53.33, $angle, $degrees, $c0, IMG_ARC_PIE);        
        $degrees = $degrees + 30;

        $endAngle = $startAngle + 360 * 1 / 12;                 // The fraction $n1 is of a complete circle        
        arc($im, 320, 320, 410, $startAngle, $endAngle, $c0);   // centreX, centreY, radius, startAngle, endAngle, colour
        $startAngle = $endAngle;
    }
...
//    echo '$angle : ' .$angle . '<br />';
//    echo '$center_pt : ' .$center_pt . '<br />';    
//    echo '$angle : '. $angle . '<br />';    
//    echo '$angle : '. ($angle / 12 ). '<br />';    
}
...
for ($i = $num_planets - 1; $i >= 0; $i--) {
    ...
    while (isset($spot_filled[(($house_num - 1) * $max_num_pl_in_each_house) + $indexy]) == 1) {
...
Function arc($image, $centreX, $centreY, $radius, $startAngle, $endAngle, $colour) {
    ...
    //imagefilledrectangle($image, $centreX + $radius * cos($angle) - 3, $centreY + $radius * sin($angle) - 3, $centreX + $radius * cos($angle) + 3, $centreY + $radius * sin($angle) + 3, $colour);

    $angle = 2 * pi() * $endAngle / 360;    // degrees converted to radians
    //imageline($image, $centreX, $centreY, $centreX + $radius * cos($angle), $centreY + $radius * sin($angle), $colour);
    //imagefilledrectangle($image, $centreX + $radius * cos($angle) - 3, $centreY + $radius * sin($angle) - 3, $centreX + $radius * cos($angle) + 3, $centreY + $radius * sin($angle) + 3, $colour);

    imagearc($image, $centreX, $centreY, 3 * $radius, 3 * $radius, $startAngle, $endAngle, $colour);
    ...
    //imagefill($image, $centreX + (4 * $radius / 3) * cos($fillAngle), $centreY + (3 * $radius / 3) * sin($fillAngle), $colour);
}
?>