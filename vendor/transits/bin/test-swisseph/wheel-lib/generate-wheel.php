...
function GetWheelImage() {        
    $retrograde = safeEscapeString(urldecode($rx1));
    
    if (get_magic_quotes_gpc()) {
        $longitude = unserialize(stripslashes(urldecode($p1)));
        $hc = unserialize(stripslashes(urldecode($hc1)));
    } else {
        $longitude = unserialize(urldecode($p1));
        $hc = unserialize(urldecode($hc1));
    }
