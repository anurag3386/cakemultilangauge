 ...
 * 
 */
class BermanBraunFinalBundleGeneraterPDF extends fpdi {
	
	//var $FranchiseName = 'BermanBraun.com';
	var $FranchiseName = 'My Year Ahead';
	var $FranchiseLink = "www.MySign.com";
	var $FranchiseId = 148;
	...
	function BermanBraunFinalBundleGeneraterPDF() {
		global $logger;
	...
	function Header() {
		...
		if($this->PageNo() > 1) {
			...
			//$this->Cell(0, 2, 'My Year Ahead '. "($Global_CurrntYear to $Global_NextYear) " , 0, 0, 'C', 0);
			//$this->Cell(0, 2, 'The My Year Ahead '. "(". date("m-d-Y", strtotime($Global_CurrntYear)) . " to " . date("m-d-Y", strtotime($Global_NextYear)) .") " , 0, 0, 'C', 0);
			$this->Cell(0, 2, 'My Year Ahead '. "(". date("M d, Y", strtotime($Global_CurrntYear)) . " to " . date("M d, Y", strtotime($Global_NextYear)) .") " , 0, 0, 'C', 0);
	...
	function Footer() {
		...
		if($this->PageNo() > 1) {
			...
			$this->SetY(-8);
			$this->Cell(0, 3, $this->FranchiseLink, 0, 0, 'L', 0);
			$this->SetY(-8);
			$this->Cell(0, 3, $this->FranchiseName, 0, 0, 'C', 0);			
			$this->SetY(-8);
			//$this->Cell(0, 3, $this->PageNo() . '/{nb}', 0, 0, 'R', 0);
			$this->Cell(0, 3, $this->PageNo(), 0, 0, 'R', 0);
...
// 		$finalBundlePDF->Ln();
	...
	/**
	 * Adding Legal Desclaimer page for BermanBraun
	 */
