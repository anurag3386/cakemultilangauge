...
class CurrentYearTransit  {
    ...
    function __Construct ($birthDTO, $ordDate) {
...
//                                                  MM = Month          DD = Day            YYYY = Year
        $this->PreviousYear = date("Y-m-d", mktime ( 0, 0, 0, $this->bMonth, $this->bDay, $this->startYear ) );
        ...
        $astrologService->calcEphemeris($ordDate->format('Y'), 4);   //Actual Code

        /* look for transits */        
        $astrologService->calcDynamicAspects($birthDTO, $this->startYear, 2);
        ...
        foreach ($astrologService->m_transit as $transit) {
            ...
            if(in_array ($transiting_planet , $CollectivePlanet)) {
                ...
                $startEndDate = $this->findTransitsStartAndEndDate($transiting_planet, $transiting_aspect, $natal_planet, $astrologService, substr($transit, 0, 10));
...
//                 }
        ...
        $astrologService->calcCrossingAspects($birthDTO, $this->startYear, 2);
...
//									     MM = Month             DD = Day               YYYY = Year
            ...
            $startEndDate = $this->findTransitsStartAndEndDate($transiting_planet, $transiting_aspect, $natal_cusp, $astrologService, substr($transit, 0, 10));
            reset($this->transit_window);
...
// 						(($transit >= $transit_lo_sep && $transit <= $transit_hi_sep) || ($transit >= $transit_hi_sep && $transit <= $transit_lo_sep))) {
    ...
    function sortTransitByPlanetNameAndAspects() {
        ...
        while (list($key, $value) = each($sortedtransits)) {
            ...
            $Global_Natal_TransitSortedList = array();
            if( $tDate >= $this->CurrntYear && $tDate < $this->NextYear ) {
                if($isThere == false) {
...
//									     MM = Month             DD = Day               YYYY = Year
    ...
    /**
     * findTransitsStartAndEndDate() function finds the Start and End date for any transiting planet
     * 
     * @param String $transiting_planet
     * @param String $transiting_aspect
     * @param String $natal_planet
     * @param Object $astrologService
     * @param String $hittingDate
     * @return multitype:multitype: number string mixed Ambigous <number, string>
     */
    function findTransitsStartAndEndDate($transiting_planet, $transiting_aspect, $natal_planet, $astrologService, $hittingDate) {
    	global $logger;
    	global $AspectRank;
    	global $NatalPlanetRank;
    	global $TransitingPlanetRank;
    	...
    	*/
    	...
    	$LastYRStartAndEndDates = array();
    	$NextYRStartAndEndDates = array();
    	$StartAndEndDates = array();
    
    	$NextYearEnd = date("Y-m-d", mktime ( 0, 0, 0, $this->bMonth, $this->bDay, $this->startYear + 2 ) );
    	$CurrentYear = date("Y-m-d", strtotime($this->CurrntYear . ' -6 month'));
    	$NextYear = date("Y-m-d", strtotime($this->NextYear . ' +6 month'));
    	//$myHittingDate =  $hittingDate;
    	$myHittingDate = date("Y-m-d", mktime ( 0, 0, 0, substr( $hittingDate, 8, 2), substr( $hittingDate, 5, 2), substr( $hittingDate, 0, 4) ));
    
    	if(is_array($astrologService->m_transit)) {
    		$i = 1;
    		foreach ($astrologService->m_transit as $transit) {
    			$trPlanet = substr($transit, 17, 4);
    			$trAspect = substr($transit, 21, 3);
    			$nPlanet = substr($transit, 24, 4);
    				
    			if(in_array ($trPlanet , $CollectivePlanet)) {
    				$tDate = trim( substr($transit, 0, 10) );
    				//									     MM = Month             DD = Day               YYYY = Year
    				$tDate = date("Y-m-d", mktime ( 0, 0, 0, substr( $tDate, 8, 2), substr( $tDate, 5, 2), substr( $tDate, 0, 4) ));
    
    				if ( $trPlanet == $transiting_planet && $trAspect == $transiting_aspect && $nPlanet == $natal_planet ) {
    
    					if (  strtotime($tDate) >= strtotime($this->PreviousYear) &&  strtotime($tDate) < strtotime($CurrentYear) ) {
    						//echo "<pre>[$i] = 1 -> [$tDate] ($myHittingDate) | ($CurrentYear) =  [$NextYear] | $transiting_planet =  $transiting_aspect =  $natal_planet </pre>";
    						array_push($LastYRStartAndEndDates, $tDate);
    					}
    					else if ( strtotime($tDate) >= strtotime($CurrentYear) && strtotime($tDate) < strtotime($NextYear) ) {
    						//echo "<pre>[$i] = 2 -> [$tDate] ($myHittingDate) | ($CurrentYear) =  [$NextYear] | $transiting_planet =  $transiting_aspect =  $natal_planet </pre>";
    						array_push($StartAndEndDates, $tDate);
    					}
    					else if ( strtotime($tDate) >= strtotime($NextYear) && strtotime($tDate) <= strtotime($NextYearEnd) ) {
    						//echo "<pre>[$i] = 3 -> [$tDate] ($myHittingDate) | ($CurrentYear) =  [$NextYear] | $transiting_planet =  $transiting_aspect =  $natal_planet </pre>";
    						array_push($NextYRStartAndEndDates , $tDate);
    					}
    					...
    					if(is_array($TransitingPlanetRank) && array_key_exists($transiting_planet, $TransitingPlanetRank)) {
    						$RankForPlanet = $TransitingPlanetRank[$transiting_planet];
    					}
    						
    					if (is_array($NatalPlanetRank) && in_array($natal_planet, $NatalPlanetRank)) {
    						$RankNumber = $AspectRank[$transiting_aspect];
    					}
    				}
    			}
    	...
    	if (  strtotime($myHittingDate) >= strtotime($this->PreviousYear) &&  strtotime($myHittingDate) < strtotime($CurrentYear) ) {
    		if(count($LastYRStartAndEndDates) > 0) {
    			$start_date = date('Y-m-d',  strtotime($LastYRStartAndEndDates[0] . ' -1 month'));
    			$end_date =  date('Y-m-d',  strtotime($LastYRStartAndEndDates[count($LastYRStartAndEndDates) - 1] . ' +1 month'));
    		} else {
    			$start_date = date('Y-m-d',  strtotime($myHittingDate . ' -1 month'));
    			$end_date = date('Y-m-d',  strtotime($myHittingDate . ' +1 month'));
    		}
    	}
    	else if ( strtotime($myHittingDate) >= strtotime($CurrentYear) && strtotime($myHittingDate) < strtotime($NextYear) ) {
    		if(count($StartAndEndDates) > 0) {
    			$start_date = date('Y-m-d',  strtotime($StartAndEndDates[0] . ' -1 month'));
    			$end_date =  date('Y-m-d',  strtotime($StartAndEndDates[count($StartAndEndDates) - 1] . ' +1 month'));
    		} else {
    			$start_date = date('Y-m-d',  strtotime($myHittingDate . ' -1 month'));
    			$end_date = date('Y-m-d',  strtotime($myHittingDate . ' +1 month'));
    		}
    	}
    	else if ( strtotime($myHittingDate) >= strtotime($NextYear) && strtotime($myHittingDate) <= strtotime($NextYearEnd) ) {
    		if(count($NextYRStartAndEndDates) > 0) {
    			$start_date = date('Y-m-d',  strtotime($NextYRStartAndEndDates[0] . ' -1 month'));
    			$end_date =  date('Y-m-d',  strtotime($NextYRStartAndEndDates[count($NextYRStartAndEndDates) - 1] . ' +1 month'));
    		} else {
    			$start_date = date('Y-m-d',  strtotime($myHittingDate . ' -1 month'));
    			$end_date = date('Y-m-d',  strtotime($myHittingDate . ' +1 month'));
    		}
