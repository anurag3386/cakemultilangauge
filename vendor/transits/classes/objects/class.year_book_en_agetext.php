...
class year_book_en_agetext extends POG_Base
{
	public $age_Id = '';
	...
	*/
	...
	function year_book_en_agetext($age_no='', $age_text='', $find_param='')
	{
		$this->age_no = $age_no;
	...
	* @return object $year_book_en_agetext
	*/
	function Get($age_Id)
	{
		$connection = Database::Connect();
		$this->pog_query = "select * from `year_book_en_agetext` where `age_id`='".intval($age_Id)."' LIMIT 1";
	...
	* @param int limit 
	* @return array $year_book_en_agetextList
	*/
	function GetList($fcv_array = array(), $sortBy='', $ascending=true, $limit='')
	{	
		$connection = Database::Connect();
		$sqlLimit = ($limit != '' ? "LIMIT $limit" : '');
		$this->pog_query = "select * from `year_book_en_agetext` ";
		$year_book_en_agetextList = Array();
		...
		while ($row = Database::Read($cursor))
		{
			$year_book_en_agetext = new $thisObjectName();
			$year_book_en_agetext->age_Id = $row['age_id'];
			$year_book_en_agetext->age_no = $this->Unescape($row['age_no']);
			$year_book_en_agetext->age_text = $this->Unescape($row['age_text']);
			$year_book_en_agetext->find_param = $this->Unescape($row['find_param']);
			$year_book_en_agetextList[] = $year_book_en_agetext;
		}
		return $year_book_en_agetextList;
	}
	...
	*/
	...
	{
		$connection = Database::Connect();
		$this->pog_query = "select `age_id` from `year_book_en_agetext` where `age_id`='".$this->age_Id."' LIMIT 1";
		$rows = Database::Query($this->pog_query, $connection);
		if ($rows > 0)
		{
			$this->pog_query = "update `year_book_en_agetext` set 
		...
		{
			$this->pog_query = "insert into `year_book_en_agetext` (`age_no`, `age_text`, `find_param` ) values (
	...
	*/
	...
	{
		$connection = Database::Connect();
		$this->pog_query = "delete from `year_book_en_agetext` where `age_id`='".$this->age_Id."'";
	...
	*/
	function DeleteList($fcv_array)
	{
		...
		{
			$connection = Database::Connect();
			$pog_query = "delete from `year_book_en_agetext` where ";
			for ($i=0, $c=sizeof($fcv_array); $i<$c; $i++)
			{
				if (sizeof($fcv_array[$i]) == 1)
				{
