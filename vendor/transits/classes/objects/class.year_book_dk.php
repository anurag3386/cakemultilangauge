...
class year_book_dk extends POG_Base
{
	public $year_book_id= '';
	...
	*/
	...
	function year_book_dk($chapter_id='', $planet_code1='', $aspect_id='', $planet_code12='', $aspect_strength='', $planet_direction='', $aspect_type='', $description='', $title='', $short_text='', $tester_text='', $chapter_name='', $find_param='')
	{
	...
	*/
	function Get($year_book_id)
	{
		...
		$this->pog_query = "select * from `year_book_dk` where `year_book_id`='".intval($year_book_enId)."' LIMIT 1";
	...
	*/
	function GetList($fcv_array = array(), $sortBy='', $ascending=true, $limit='')
	{
		...
		$this->pog_query = "select * from `year_book_dk` ";
...
//		echo '<pre>' .$this->pog_query . '</pre>';		
	...
	*/
	...
	{
		$connection = Database::Connect();
		$this->pog_query = "select `year_book_id` from `year_book_dk` where `year_book_id`='".$this->year_book_id."' LIMIT 1";
		$rows = Database::Query($this->pog_query, $connection);
		if ($rows > 0)
		{
			$this->pog_query = "update `year_book_dk` set 
		...
		{
			$this->pog_query = "insert into `year_book_dk` (`chapter_id`, `planet_code1`, `aspect_id`, `planet_code12`, `aspect_strength`, `planet_direction`, `aspect_type`, `description`, `title`, `short_text`, `tester_text`, `chapter_name`, `find_param` ) values (
	...
	*/
	...
	{
		$connection = Database::Connect();
		$this->pog_query = "delete from `year_book_dk` where `year_book_id`='".$this->year_book_id."'";
	...
	*/
	function DeleteList($fcv_array)
	{
		...
		{
			$connection = Database::Connect();
			$pog_query = "delete from `year_book_dk` where ";
			for ($i=0, $c=sizeof($fcv_array); $i<$c; $i++)
			{
				if (sizeof($fcv_array[$i]) == 1)
				{
