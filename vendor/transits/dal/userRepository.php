...
// 		return '';
	...
	public function GetUserDetail($user_id, $userType='') {
		if (!empty($userType)) { // For another person
			$userId = explode('_', $user_id);
			$SQLQuery = "SELECT * from another_persons where another_persons.id=".$userId['1'];
	...
	public function GetUserBirthDetailByUserId($user_id, $userType='') {
		/*$SQLQuery = "SELECT
					...
					WHERE UserId = :UserId ";*/
		if (!empty($userType)) {
			$selectedUserId = explode('_', $user_id);
			$SQLQuery = "SELECT
						another_persons.*,
					...
					WHERE another_persons.id = ".$selectedUserId[1];
		} else {
			$SQLQuery = "SELECT
						birth_details.*,
						cities.longitude, cities.latitude, cities.city as cityname, cities.county as state, cities.typetable,
