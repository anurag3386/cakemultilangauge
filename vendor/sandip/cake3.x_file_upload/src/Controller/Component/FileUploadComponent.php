...
class FileUploadComponent extends Component{
    ...
    function doFileUpload($file_data){
        ...
        if (!empty($file_data['custom_index']) && ($file_data['custom_index'] == 'elite-cover-page')) {
            $ext = trim(substr(strrchr($file_data['name'],'.'),1));
            $user_id = !empty ($this->request->session()->read('Auth.User.id')) ? $this->request->session()->read('Auth.User.id') : time();
            $new_file_name = $user_id.'.'.$ext;
        }
        elseif($type[0] == 'text')
        {
          $new_file_name = $file_data['name'];   
        } 
        elseif($type[0] == 'audio')
        {
           $new_file_name = $file_data['name'];     
