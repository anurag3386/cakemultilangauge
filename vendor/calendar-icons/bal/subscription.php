<?php 
if(!require_once(BALPATH . "/include.php")) {
...
if(!class_exists("Subscription")) {
	class Subscription
	{
		...
		public function IsSubscribeUser($user_id, $userType) {
			$subscriptionRepository = new SubscriptionRepository();
			try {
				return $subscriptionRepository->IsSubscribeUser($user_id, $userType);
			} catch(Exception $ex) {
				die($ex->getMessage());
			}
		}
		 ...
		 */
		public function customUserDetail ($user_id, $userType) {
			$subscriptionRepository = new SubscriptionRepository();
			try {
				return $subscriptionRepository->GetSubscriptionUserDetail ($user_id, $userType);
			} catch(Exception $ex) {
				die($ex->getMessage());
			}
		}
