...
// 						WHERE sp.subscription_id ='".$subscription_id."'";
		...
		public function IsSubscribeUser($user_id, $userType)
		{
			try
			{
				$obj = new cDatabase();
				...
				if (!empty($userType)) {
					$sql = " SELECT * FROM subscribes where status = 1 and user_id = '".$user_id."' and user_type = 'anotherPerson' and start_date <= '".time()."' and end_date >= '".time()."'";
				} else {
					$sql = " SELECT * FROM subscribes where status = 1 and user_id = '".$user_id."' and user_type = 'user' and start_date <= '".time()."' and end_date >= '".time()."'";
				}
		...
		/**
		 * Custom function for get full detail of calendar subscribed user
		 * Created By : Krishna Gupta
		 * Created Date : Jan 17, 2017
		 */
		public function GetSubscriptionUserDetail ($userId, $userType)
		{
			try
			{
				$obj = new cDatabase();
				$user_type = 'user';
				if (!empty($userType)) {
					$user_type = 'anotherPerson';
				} 
				$sql = " SELECT * FROM subscribes where user_id = '".$userId."' and user_type = '$user_type' and end_date >= '".time()."'";
				$sql .= " ORDER BY id DESC LIMIT 1";
				$query = $obj->db->read($sql);
				return $query;
			}
			catch(Exception $ex)
			{
