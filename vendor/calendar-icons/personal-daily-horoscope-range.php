...
<input type="hidden" id="person_id" name="person_id" value="<?php echo $person_id;?>" />
	<?php
        //if( $_SERVER['REMOTE_ADDR'] == '103.254.97.14' || $_SERVER['REMOTE_ADDR'] == '103.248.117.12' ) {
            if (date('d', ($startDateInMS+86400)) == '01' || date('d', ($startDateInMS+86400)) == '1') {
                $subscriptionDetail['start_date'] = strtotime('01-'.date('m-Y', $startDateInMS+86400));
                $subscriptionDetail['end_date'] = strtotime(date('t-m-Y', $endDateInMS+86400));
            } else {
        ...
        $_SESSION['calendar_start_date'] = ($startDateInMS+86400);
        $_SESSION['calendar_end_date'] = ($endDateInMS+86400);
        ...
        $monthLimit = $total_months = $diff; //($last_month - $first_month); die;

        
        $first_month_date = date('Y-m', $subscriptionDetail['start_date']).'-01';
        $num_months = $total_months;
        $current_month = $first_month_date; //date('Y-m').'-01';
        ...
        if (isset($monthNo) && !empty($monthNo)) {
            ...
            if (strtotime('01-'.$explodedPDate[0].'-'.$explodedPDate[1]) <= $startDateInMS) {
                $PDate = date('m-d-Y', ($startDateInMS+86400));
            } else {
                $PDate = $explodedPDate[0].'-01-'.$explodedPDate[1];
            }

        } else {
            $currYear = date ('Y', $subscriptionDetail['start_date']);
            $currMonth = date ('m', $subscriptionDetail['start_date']);
            $monthNo = (($currYear - $year1) * 12) + ($currMonth - $month1);
            if (!$monthNo) {
                $monthNo = 1;
            }
            $PDate = date('m-d-Y', ($startDateInMS+86400)); //date('m-d-Y', $subscriptionDetail['start_date']);
        }
        //echo '<br>'.$PDate; die;
        $nextMonth = $monthNo+1;

        $monthLimit = $monthLimit+1;
        ...
        //if ($IsSubscribedUser) {
            $calendarResponse .= "<div class='tabify04-outer'>
        		<ul id='tabbed04' class='tabify04'>";
                    if ($monthNo > 1) {
                        $calendarResponse .= "<li class='previousMonth' onclick='nextMonthDataInrange(".$startDateInMS.','.$endDateInMS.','.($monthNo-1).")'> <img src=".BASEURL."webroot/img/left.png> </li>";
                    }
                    $i = 1;
                    
                    //$calendarResponse .= "<li class='tablinks active' ref='".$monthNo."'>".date('F - Y', strtotime($first_month_date))."</li>";
                    $mName = $MonthNameArray[date('F', strtotime($first_month_date.' + '.($monthNo-1).' Months'))];
                    $calendarResponse .= "<li class='tablinks active' ref='".$monthNo."'>".$mName.' - '.date('Y', strtotime($first_month_date.' + '.($monthNo-1).' Months'))."</li>";

                    if ($monthNo < $monthLimit) {
                        $calendarResponse .= "<li class='nextMonth nayamonth' onclick='nextMonthDataInrange(".$startDateInMS.','.$endDateInMS.','.$nextMonth.")' ref='2'> <img src=".BASEURL."webroot/img/right.png> </li>";
                    }
        		$calendarResponse .= "</ul>
                ...
                if($i < $startday) {
        			...
        			if(($i % 7) == 6 ) {
                        ...
                        if (validDateForShowPrediction($dateWithLink)) {
                            ...
                            $calendarResponse .= "<a class='lnkDaily' id='".$dateWithLink."' href='javascript:void(0);' onclick='outOfRangeMsg();'>";
                        }
    ...
    function validDateForShowPrediction ($selectedDate) {
        ...
        /*if ($selectedDate >= ($_SESSION['calendar_start_date']-86400)) {
            echo 'Greater than start date';
        }

        if (($_SESSION['calendar_end_date']) >= $selectedDate) {
            echo '<br>Less than start date';
        }

        if ($selectedDate >= ($_SESSION['calendar_start_date']-86400) && ($_SESSION['calendar_end_date']) >= $selectedDate) {
            echo '<br> Both true';
        }
        echo '<hr><hr>';*/
        /*die;
        ...
        echo '<br>selected Date => '.$selectedDate.'<br>Session selected Date => '.$_SESSION['calendar_start_date'].'<br>selected Date => '.date('d-m-Y', $selectedDate).'<br>Session selected Date => '.date('d-m-Y', $_SESSION['calendar_start_date']).'<br>End Time => '.$_SESSION['calendar_end_date'].'<br>End Time => '.date('d-m-Y', $_SESSION['calendar_end_date']); die;*/
        if ($selectedDate >= ($_SESSION['calendar_start_date']-86400) && ($_SESSION['calendar_end_date']) >= $selectedDate) {
            return true;
        }
        return false;
