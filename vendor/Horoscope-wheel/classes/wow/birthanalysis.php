<?php
/*
 * Script: class.astrolog.wow.birthanalysis.php
...
/*
 * the log4j logging platform for PHP is used here
 * define the appender to use
 * - echo dumps logging details to the page
 * - db (mysql) dumps logging into the database (prune regularly if set to debug)
 * look at remote chainsaw monitoring
*/
 ...
 * Birthdata class
 *
 * The BirthAnalysis class produces a packed data recordset for onward analysis and
 * report generation.
 *
 * @author Andy Gray <andy.gray@astro-consulting.co.uk>
 ...
 * @version 1.0
 */
class BirthAnalysis extends AstrologChartAPI {
    ...
    function BirthAnalysis( $data, $debug = false ) {
        ...
        $logger->debug("BirthAnalysis::BirthAnalysis");
        ...
        if( $this->m_timed_data === true ) {
            $logger->debug("BirthAnalysis::BirthAnalysis - using timed data");
        } else {
            $logger->debug("BirthAnalysis::BirthAnalysis - using untimed data (solar chart)");
        }
    ...
    function packAscendant() {
        ...
        $logger->debug("BirthAnalysis::packAscendant");
        $this->m_chapter = 0;
        $degree = floatval($this->m_object['cusp'][1]);
        $logger->debug("BirthAnalysis::packAscendant, degree = $degree");
    ...
    function packPlanets() {
        ...
        $logger->debug("BirthAnalysis::packPlanets");
        for( $planet = 1; $planet < 12; $planet++ ) {
            $this->m_chapter = $planet;
            $logger->debug("BirthAnalysis::packPlanets - planet is ".$planets[$planet]);
    ...
    function packAscInSign() {
        ...
        $logger->debug("BirthAnalysis::packAscInSign");
        $this->m_planet = 12;
        $this->m_aspect_ref = "200";
        $this->m_aspect_orb = 0.0;
        $this->m_record_context = 1;
        $degree = floatval($this->m_object['cusp'][1]);
        $logger->debug("BirthAnalysis::packAscInSign, degree = $degree");
        $sign = intval( (floatval($degree) / 30.0 ) );
        $logger->debug("BirthAnalysis::packAscInSign, sign = $sign");
    ...
    function packAscInAspect() {
        ...
        $logger->debug("BirthAnalysis::packAscInAspect");
    ...
    */
    function packPlanetInSign($planet /* 1..10 */) {
        ...
        //ogger->debug("BirthAnalysis::packPlanetInSign($planet) => $this->m_planets[$planet]");
        $this->m_planet = ($planet-1);
        $this->m_aspect_ref = "200";
        $this->m_aspect_orb = 0.0;
        $this->m_record_context = 1;
        $degree = floatval($this->m_object[$this->m_planets[$planet]]['longitude']);
        $logger->debug("BirthAnalysis::packPlanetInSign(degree = $degree)");
        $sign = intval( (floatval($degree) / 30.0 ) );
        $logger->debug("BirthAnalysis::packPlanetInSign(sign = $sign)");
        $logger->debug("BirthAnalysis::packPlanetInSign(retrograde = [".$this->m_retrograde."])");
        $this->m_record_context_value = sprintf("%02d", $sign);
        $this->packRecord();
    }
    ...
    function packPlanetInHouse($planet) {
        ...
        $logger->debug("BirthAnalysis::packPlanetInhouse($planet)");
        $this->m_aspect_ref = "200";
        $this->m_aspect_orb = 0.0;
    ...
    function packPlanetInAspect($planet) {
        ...
        $logger->debug("BirthAnalysis::packPlanetInAspect($planet)");
        for( $aspect = 0; $aspect < count($this->m_aspect); $aspect++ ) {
    ...
    */
    function packRecord() {
        ...
        $logger->debug("BirthAnalysis::packRecord");
		 ...
		 * valid set = 1..3
        */
        $logger->debug("BirthAnalysis::packRecord - aspect=$this->m_aspect_ref");
        $logger->debug("BirthAnalysis::packRecord - aspect orb = ".$this->m_aspect_orb);
        if( substr($this->m_aspect_orb,0,1) == '+' ) {
            $logger->debug("BirthAnalysis::packRecord - aspect is applying");
        } else {
            $logger->debug("BirthAnalysis::packRecord - aspect is separating");
        }
        ...
        */
        switch( $this->m_aspect_ref ) {
            ...
            /*
				 ...
				 * - weak >= 5.0
            */
                $logger->debug("BirthAnalysis::packRecord - conjunction");
                $aspect_type = 0;
                switch( intval( substr($this->m_aspect_orb,1,1) ) ) {
                    case 0: case 1:
                        $logger->debug("BirthAnalysis::packRecord - aspect is strong");
                        $aspect_strength = 0;
                        break;
                    case 2: case 3: case 4:
                        $logger->debug("BirthAnalysis::packRecord - aspect is medium");
                        $aspect_strength = 1;
                        break;
                    default:
                        $logger->debug("BirthAnalysis::packRecord - aspect is weak");
                        $aspect_strength = 2;
                        break;
                }
                break;
        ...
        */
        ...
        $logger->debug("BirthAnalysis::packRecord - planet(".($this->m_planet+1)."), context(".$this->m_record_context."), duplicate(".$duplicate.")");
        ...
        $logger->debug("BirthAnalysis::packRecord - record = ".
                sprintf
                ( "%05d-%04d-%03d-%02d%02d-%d-%d-0000000000000000000000-%03d-%02d-%03d",
                $this->m_chapter + 10001,
                $this->m_planet + 1000,
                $this->m_aspect_ref,
                $this->m_record_context,
                ...
                $aspect_strength,		/* aspect strength	{ 0 => strong,		1 => medium,	2 => weak		}	*/
                $aspect_type,			/* aspect type		{ 0 => conjoins,	1 => positive,	2 => negative	}	*/
                $this->m_record_num-1	/* takes in to account the previous autoincrement							*/
                )
        );
    ...
    function sortAspects() {
        ...
        global $logger;
        $logger->debug("BirthAnalysis::sortAspects");
        $sortedaspects = array();
        for( $aspect = 0; $aspect < count($this->m_aspect); $aspect++ ) {
    ...
    */
    function updatePageNo( $pno ) {
        $this->m_analysis_context =
                sprintf("%s%03d%s",
                substr($this->m_analysis_context, 0, ($this->m_analysis_context_index + 18)),
                intval($pno),
                substr($this->m_analysis_context,
                ($this->m_analysis_context_index + 21),
                ( strlen($this->m_analysis_context) - ($this->m_analysis_context_index + 21) )
                )
        );
    }
};
