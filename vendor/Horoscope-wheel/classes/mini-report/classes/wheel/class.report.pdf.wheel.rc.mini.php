<?php
/*
 * File  : class.report.pdf.wheel.rc.mini.php
* Author: Parmar Amit <amit@ntechcorporate.com>
*
* Description
...
class PDF_Wheel extends PDF_Bookmark
{
	...
	var $table_Sun_In_House;
	var $table_Moon_In_House;
	var $table_ACS_In_House;
	...
	function drawTableUserInfo($lang) {
		...
		$UserTableInfoHeight = 14;		
		
		$this->Rect(
				$this->table_user_info_origin_x, $this->table_user_info_origin_y, $this->table_user_info_width, $UserTableInfoHeight
		);
		...
		$this->Line(
				...
				($this->table_user_info_origin_x + ($this->table_user_info_width / 4)), 
				($this->table_user_info_origin_y + $UserTableInfoHeight)
		);
		/* 50% */
		$this->Line(
				...
				$this->table_user_info_origin_y, 
				($this->table_user_info_origin_x + ($this->table_user_info_width / 2)), 
				($this->table_user_info_origin_y + $UserTableInfoHeight - 7)
		);
		/* 75% */
		$this->Line(
		...
		*/
		// fname
		$this->SetXY($this->table_user_info_origin_x, $this->table_user_info_origin_y);
		$this->MultiCell(($this->table_user_info_width / 4), ($this->table_user_info_height / 3), $this->table_user_info_fname, 0, 2, 'C', 0);
	
		// lname OLD
		// Date Of Birth
		$this->SetXY($this->table_user_info_origin_x, $this->table_user_info_origin_y + ($this->table_user_info_height / 3));
		...
		$this->Cell(($this->table_user_info_width / 4), ($this->table_user_info_height / 3), chr(184) . " in " . chr($this->symbol_map[$this->table_Sun_In_House]), 0, 2, 'C', 0);
			
		// Orb Setting
		//MOON IN
		$this->SetXY($this->table_user_info_origin_x + (3 * $this->table_user_info_width / 4), $this->table_user_info_origin_y);
		$this->SetFont($this->table_planet_info_font_family, $this->table_user_info_font_weight, $this->table_user_info_font_size);
		$this->Cell(($this->table_user_info_width / 4), 
				($this->table_user_info_height / 3), 
				chr(155) . " in " . chr($this->symbol_map[$this->table_Moon_In_House]), 
				0, 2, 'C', 0);			
		
		$this->SetXY($this->table_user_info_origin_x, $this->table_user_info_origin_y + 5);
		$this->Cell(($this->table_user_info_origin_x + ($this->table_user_info_width) + ($this->table_user_info_width / 3)), 
				($this->table_user_info_height / 2), 
				sprintf("%s", $this->table_user_info_house_system), 
				0, 2, 'C', 0);

		//ASC In
		/*
		$this->SetFont($this->table_planet_info_font_family, $this->table_user_info_font_weight, $this->table_user_info_font_size);
		$this->Cell(($this->table_user_info_width / 4), ($this->table_user_info_height / 3), chr(124) . " in ". chr($this->symbol_map[$this->table_ACS_In_House]), 0, 2, 'C', 0);
		$this->SetFont($this->table_user_info_font_family, $this->table_user_info_font_weight, $this->table_user_info_font_size);
		*/
	}
	
	/**
	 * This will draw Symbol information
	 * @param string $lang
	 */
