...
class Report {
	 ...
	 * </ul>
	 */
	function Report($Name = '', $Language = 'en', $OrderId) {
	...
	function Run( $AnalysisContext, $Book,  $Generator) {
		...
		$Introduction_Generic_Text[$this->Language]["Introduction"] = sprintf($Introduction_Generic_Text[$this->Language]["Introduction"], $this->UserName); 

		$this->Generator->ReportHeader($Introduction_Generic_Text[$this->Language]["BirthReport"]);
		...
		while( $this->Analysis_Context->EOF() === false ) {			
			...
			echo "<pre>$rTotalRecord :: $rIndex ::: == $content</pre>";				
			/** 
			...
			if( $rIndex == 1 && $this->ReportType == "mini-b") {	//This is Aspect
				$ContextType = 'dynamic';
			}
			
			//echo "<pre>***************$chapter | $content | $ContextType == [ $ContextIndex ]</pre>";				
		...
		$this->Generator->GenericSummary($this->CurrentOrderID);
	}
		...
		if( $chapter >= 0 && (strtolower($ContextType) == "static" && ($this->ReportType == "mini-a" || $this->ReportType == "mini-b")) ) {		
			...
			if($sIndex != "") {
				if(intval($sIndex) <= 1005) {
					/** Printing out Planet Generic Text */
					$this->Generator->GenericIntroductionSection( $sIndex, $oIndex );
				}
			}
		}
		
		if( strtolower($ContextType) == "dynamic" && $this->ReportType == "mini-b") {
			/** Printing out Planet Generic Text */
			$sectionContent = substr($content, (0*48), 48);
			...
			$this->Generator->GenericAspectIntroductionSection($sIndex, $cIndex, $oIndex);
		}
		
		if( strtolower($ContextType) == "dynamic-t" && $this->ReportType == "mini-b") {
			/** Printing out Planet Generic Text */
			$sectionContent = substr($content, (0*48), 48);
			...
			$StartDate = $this->Analysis_Context->getTransitStartDate( $sIndex, $oIndex, $cIndex ); 		/* dd/mm/YYYY */			
			$EndDate = $this->Analysis_Context->getTransitEndDate( $sIndex, $oIndex, $cIndex );	 		/* dd/mm/YYYY */
	...
	function analyseChapter( $chapter, $content ) {
		...
		if( $this->Context == "dynamic-t" && $this->ReportType == "mini-b") {
			/** Printing out Planet Generic Text */
			$sectionContent = substr($content, (0*48), 48);
			$sIndex = $this->getSectionHeaderSubject( $sectionContent );
	...
	function analyseSection( $content ) {
		...
		//echo sprintf("<pre>-> %s : %s : %s <-</pre>", $sIndex, $cIndex, $oIndex);
		/**
		...
		*/
		...
		if( $this->ReportType == "y3" || ($this->ReportType == "pc3" && $this->isDynamicAspect($content)) ) { /* *** test suggestion *** */
			if( $oIndex != 1014 /* IC */ && $oIndex != 1015 /* DC */ ) {
				$this->Generator->SectionHeader (
		...
		if( $this->ReportType == "y3" || ($this->ReportType == "pc3" && $this->isDynamicAspect($content)) ) { /* *** test suggestion *** */
			if( $oIndex != 1014 /* IC */ && $oIndex != 1015 /* DC */ ) {
				...
				$this->Generator->SectionSubHead( $StartDate, $EndDate );
			}
		}
		...
		switch( substr($checkOIndex,0,2 ) ) {
			...
			case "10":
				...
				//if( $this->ReportType == "y3" || ( $this->ReportType == "pc3" && $this->isDynamicAspect($content)) ) {
				...
				if( $this->ReportType == "mini-b" && $this->isDynamicTransit($content) ) {
					...
					if( ( intval( $subject ) >= 6 /* Jupiter */ && intval ( $subject ) <= 10 /* Pluto */ ) ) {
						if( intval($object) == 15 /* IC */|| intval($object) == 16 /* DC */ ) {
							...
							//Report::analyseSection - transiting planet ($subject) in aspect to natal planet ($subject)
							$this->analyseAspect( $subject, $object, $this->isDynamicTransit($content) );
						}
					}
				} else {
	...
	function analyseAspect($subject, $object, $dynamic_context) {
		...
		if( $dynamic_context === false ) {
			...
			*/
			if( $this->ReportType == "mini-a" || $this->ReportType == "mini-b") {
				/***** HACKHACK *** in static context for career, pc or pc3 **/
				$book = "01";
				if( $duplicate === false ) {
					$this->Generator->GenericAspectIntroductionMergingText();					
					$attitude = $this->getBookHeading( $book, $chapter, $code3, $code4, $this->Language );
					$description = $this->getBookParagraph( $book, $chapter, $code3, $code4, $this->Language );
					$this->Generator->SectionContent( "PROFESSIONAL", $attitude, $description );
				} else {
					$this->Generator->CrossReference( $subject, $connector, $object );
				}
			}
		...
		if( $dynamic_context === true && $this->ReportType == "mini-b" ) {
			...
			*/
			...
			$this->Generator->GenericTransitIntroductionMergingText();
