...
//class TransitCalculation extends CommonPDFHelper {
class GenerateTransitForMiniReport extends CommonPDFHelper {
	...
	var $CollectivePlanet = array("1005", "1006", "1007", "1008", "1009");
...
// 		$this->CurrntYear = date("Y-m-d", mktime ( 0, 0, 0, $this->bMonth, $this->bDay, $this->startYear ) );
		...
		$this->PreviousYear = $GLOBALS["Global_PreviousYear"];
		$this->NextYear = $GLOBALS["Global_NextYear"];
		$this->CurrntYear = $GLOBALS["Global_CurrntYear"];
		...
		if(is_array($astrologService->m_transit)) {
			foreach ($astrologService->m_transit as $transit) {
				...
				if($natal_planet != "1001" && $natal_planet != "1012" && $natal_planet != "1013") 
				{
					if(in_array ($transiting_planet , $CollectivePlanet)) {
						...
						//echo $tDate . " == " .date("Y-M-d", strtotime($tDate)) . " == " . trim( $transit ) . "<br />";
		...
		$astrologService->calcHouseCrossingAspects($birthDTO, $this->startYear - 2, 5);
		$astrologService->SAandJUCrossing = $this->GetCrossingPoints($astrologService->SAandJUCrossing);
		$this->GenerateHouseCrossingList($astrologService->SAandJUCrossing);
		//print_r($astrologService->SAandJUCrossing);
	...
	public function GetCrossingPoints ($SAandJUCrossing) {
		$FinalArray = array();
		$JULastEnter = "";
		$SULastEnter = "";
		
		foreach($SAandJUCrossing as $Key => $Item) {
			if(floatval($Item["Jupiter"]["position"]) >= 0.0 && floatval($Item["Jupiter"]["position"]) <= 0.99) {				
				//$RemoveDuplicate[substr($FinalArray[$index], 17, 11)]				
				if($JULastEnter != $Item["Jupiter"]["signno"]) {
					$FinalArray[] = $this->GetFormatedSAJU($Key, "1005", $Item["Jupiter"]);
					$JULastEnter = $Item["Jupiter"]["signno"];
				}
			}

			if(floatval($Item["Saturn"]["position"]) >= 0.0 && floatval($Item["Saturn"]["position"]) <= 0.99) {
				if($SULastEnter != $Item["Saturn"]["signno"]) {
					$FinalArray[] = $this->GetFormatedSAJU($Key, "1006", $Item["Saturn"]);
					$SULastEnter = $Item["Saturn"]["signno"];
				}
			}
		}
	...
	public function GetCrossingPoints_OLD ($SAandJUCrossing) {
		$FinalArray = array();

		foreach($SAandJUCrossing as $Key => $Item) {
			if(floatval($Item["Jupiter"]["position"]) >= 0.0 && floatval($Item["Jupiter"]["position"]) <= 0.99) {
				//$FinalArray[$Key]["Jupiter"]= $Item["Jupiter"];
				$FinalArray[] = array("position" =>  floatval($Item["Jupiter"]["position"]),
						"item" => $this->GetFormatedSAJU($Key, "1005", $Item["Jupiter"]));
			}
				
			if(floatval($Item["Saturn"]["position"]) >= 0.0 && floatval($Item["Saturn"]["position"]) <= 0.99) {
				//$FinalArray[$Key]["Saturn"] = $Item["Saturn"];
				$FinalArray[] = array("position" =>  floatval($Item["Saturn"]["position"]),
						"item" => $this->GetFormatedSAJU($Key, "1006", $Item["Saturn"]));
			}
		}
		$RemoveDuplicate = array();
		$TestArray = array();

		for ($index = 0; $index < count($FinalArray); $index++) {
			//- 123456789012345678901234567890
			//- 2012-01-01 00:00 10050001010 D
			//$RemoveDuplicate[substr($FinalArray[$index], 17, 11)] = $FinalArray[$index];
			$RemoveDuplicate[substr($FinalArray[$index]["item"], 0, 10)] = $FinalArray[$index];
			$TestArray["position"][$index] = intval($FinalArray[$index]["position"]);
			$TestArray["cdate"][$index] = substr($FinalArray[$index]["item"], 0, 10);
		}
		
		//
		array_multisort($TestArray["cdate"], SORT_ASC, $TestArray["position"], SORT_ASC, $RemoveDuplicate);

		unset($FinalArray);
		$FinalArray = array();
		reset($RemoveDuplicate);
		
		while (list($key, $value) = each($RemoveDuplicate)) {
			array_push($FinalArray, $value["item"]);				
		}
		
		return $FinalArray;
	}

	protected function GetFormatedSAJU($cDate, $Planet, $Item) {
		global $HouseCuspLongitude;

		$HouseNO =  sprintf("%04d", $HouseCuspLongitude[$Item["signno"]] + 1);
		$SignName =  trim($Item["signname"]);
		//$IsRetrograde = (trim($Item['retrograde']) == '.' ? 'R' : 'D');
		$IsRetrograde = ($Item['retrograde'] ? 'R' : 'D');
		
		return sprintf("%04d-%02d-%02d 00:00 %04d000%04d %s %s",
				/* date			 */ intval(substr($cDate, 0, 4)), intval(substr($cDate, 4, 2)), intval(substr($cDate, 6, 2)),
				/* transiting	 */ $Planet,
				/* natal		 */ sprintf("%04d", $HouseNO),
	...
	function GenerateHouseCrossingList($CrossingList) {
		...
		foreach ($CrossingList as $transit) {
			...
			$SignName = trim(substr($transit, 30, 3));

			$tDate = trim( substr($transit, 0, 10) );
			//									     MM = Month             DD = Day               YYYY = Year
			//$tDate = date("Y-m-d", mktime ( 0, 0, 0, substr( $tDate, 8, 2), substr( $tDate, 5, 2), substr( $tDate, 0, 4) ));
			$tDate = date("Y-m-d", mktime ( 0, 0, 0, substr( $tDate, 5, 2), substr( $tDate, 8, 2), substr( $tDate, 0, 4) ));
			...
			if($tDate >= $Global_CurrntYear && $IsRetrograde == 'D') {
				...
				if($transiting_planet == '1005') {
					...
					if($tDate >= $Global_CurrntYear) {
						$JUArray = $this->AddTOSAandJUArray($JUArray, $transiting_planet, $transiting_aspect, $natal_cusp, $tDate, $EndDate['enddate'], $IsRetrograde, $EndDate['stayinhouse'], $SignName);
					}
				}
				else
				{
					$SAArray = $this->AddTOSAandJUArray($SAArray, $transiting_planet, $transiting_aspect, $natal_cusp, $tDate, $EndDate['enddate'], $IsRetrograde, $EndDate['stayinhouse'], $SignName);
				}
			}
		}
		...
		foreach ($JUArray as $Key => $Item) {
			if($Key == 0) {
				...
				if($StartDate['startdate'] != '') {
					$JUArray = $this->AddTOSAandJUArray($JUArray, $PT, $ASP, $PN, $StartDate['startdate'], $start, $IsRetrograde, 'N', $SignName);
				}
				break;
			}
		}
		...
		foreach ($SAArray as $Key => $Item) {
			if($Key == 0) {
				...
				if($StartDate['startdate'] != '') {
					$SAArray = $this->AddTOSAandJUArray($SAArray, $PT, $ASP, $PN, $StartDate['startdate'], $start, $IsRetrograde, 'N', $SignName);
				}
				break;
			}
		}
	...
	function GetLeavesDateForCups($CrossingList, $Planet, $CuspNo) {
		...
		foreach ($CrossingList as $transit) {
			...
			if($transiting_planet == $Planet && intval($natal_cusp) ==  (intval($CuspNo) + 1)) {
				$tDate = trim( substr($transit, 0, 10) );					
				$NewDate = explode("-", $tDate);
				
				//									     MM = Month             DD = Day               YYYY = Year
				//$tDate = date("Y-m-d", mktime ( 0, 0, 0, substr( $tDate, 8, 2), substr( $tDate, 5, 2), substr( $tDate, 0, 4) ));
				$tDate = date("Y-m-d", mktime ( 0, 0, 0, $NewDate[1], $NewDate[2], $NewDate[0] ));
	...
	function AddTOSAandJUArray($SAandJU, $TPlanet, $TAspect, $NatalCusp, $TSDate, $TEDate, $IsRetrograde, $IsStayInHouse, $SignName){
	...
	*/
	...
	function AddGraphtoPDF() {
		...
		$bDate = $Global_CurrntYear;
		$bDate .= ' to ' . $Global_NextYear;
			
		$this->SetFont('wows', '', 14);
		//$this->Cell($this->TableCellWidth, $this->TableHeight, 'Mr Urvish - Transit aspects ( ' . $bDate . ' )', 0, 0, 'L', false);
		$this->Cell($this->TableCellWidth * 4, $this->TableHeight, $CustomerName, 0, 0, 'L', false);
		$this->Ln();
		
		$this->Cell($this->TableCellWidth * 4, $this->TableHeight, 'Trend Start from ( ' . $bDate . ' )', 0, 0, 'L', false);
		$this->Ln();
