...
class UserRepository {
	...
	public function validateUserLogon($userName,$password) {
		$SQLQuery = "SELECT id,Status,UserGroupId,UserName FROM `user` o WHERE UserName = :UserName and Password = :Password";
...
// 		return '';
	...
	public function SaveUserProfile($data) {
		...
		$SQLQuery = "INSERT INTO `profiles`
				(user_id, first_name, last_name, Gender, Phone, Address, City, State, Country, Zip, CreatedDate, ModifiedDate, CreatedBy, ModifiedBy)
			VALUES
	...
	public function SaveUserBirthDetail($data) {
		...
		$SQLQuery = "INSERT INTO `userbirthdetail`
			...
			VALUES
				(:UserId, :Day, :Month, :Year, :Hours, :Minutes, :Seconds, :unTimed, :GMT, :ZoneRef, :SummerTimeZoneRef, :Longitute, :Lagitute, :CreatedDate, :ModifiedDate, :CreatedBy, :ModifiedBy, :country_name, :sunsign, :city, :country, :state)";
		*/
		$SQLQuery = "INSERT INTO `birth_details`
				(user_id, Day, Month, Year, Hours, Minutes, unTimed, GMT, ZoneRef, SummerTimeZoneRef, Longitute, Lagitute, CreatedDate, ModifiedDate, CreatedBy, ModifiedBy, country_name, sunsign, city, country, state)
			VALUES
	...
	public function SaveUserHororyDetail($data) {
		...
		$SQLQuery = "INSERT INTO `userhorory`
				(user_id, Address, City, State, Country, CreatedDate, ModifiedDate, CreatedBy, ModifiedBy,
	...
	public function UpdateUserProfile($data) {
		$SQLQuery = "UPDATE `profiles` SET
						first_name = :FirstName, last_name = :LastName, Gender = :Gender, Phone = :Phone,
						Address = :Address, City = :City, State = :State, Country = :Country, Zip = :Zip, 
						ModifiedDate = :ModifiedDate, ModifiedBy = :ModifiedBy
			WHERE id = :UserProfileId AND user_id = :UserId ";
	...
	public function UpdateUserBirthDetail($data) {
		...
		/*$SQLQuery = "UPDATE `birth_details` SET
			...
			WHERE user_id = :UserId and UserBirthDetailId = :UserBirthDetailId";*/
	...
	public function UpdateUserHororyDetail($data) {
		...
		$SQLQuery = "UPDATE `userhorory` SET
			...
			WHERE UserHororyId = :UserHororyId AND user_id = :UserId ";
	...
	public function GetUserProfileDetailByUserId($user_id) {
		/**
		 * Query changed according to our requirement and DB structure
		 * By : Krishna Gupta
		 * Date : Nov. 21, 2016
		 */
		/* $SQLQuery = "SELECT
					id,user_id, first_name, last_name, Gender, Phone, Address, City, State,
					Country, Zip, CreatedDate, ModifiedDate, CreatedBy, ModifiedBy */
			$SQLQuery = "SELECT
					id,user_id, first_name, last_name, Gender, Phone, Address, city_id,
					country_id, zip, created, modified
			FROM `profiles`
			WHERE user_id = :UserId ";
	 ...
	 * Modified Date : Nov. 16, 2016
	 */
	public function GetUserDetail($uid, $user_type) {
		$user_id = !empty($uid) ? $uid : '';
		$user_type = !empty($user_type) ? ($user_type) : '';
		if (!empty($user_id)) {
			if (!empty($user_type)) {
				$SQLQuery = "SELECT another_persons.*, cities.*, languages.name as language, languages.code as language_code
								FROM another_persons
								JOIN cities ON cities.id = another_persons.city_id
								JOIN languages ON languages.id = another_persons.language_id
	...
	public function GetUserDetailForMail($user_id) {
		$SQLQuery = "SELECT
				u.UserName,up.first_name, up.LastName, up.Gender
	...
	public function CheckPassword($user_id, $password) {
		$SQLQuery = "SELECT count(id) as count FROM `user` WHERE Password = :Password AND id = :UserId ";
	...
	public function ChangePassword($user_id, $password) {
		$SQLQuery = "UPDATE `user` SET Password = :Password WHERE id = :UserId ";
	...
	public function GetUserToAssignPreviewReport() {
		$obj = new cDatabase();
		$sql = " SELECT id FROM `user` WHERE `preview_report` = '' AND UserId !=1";
	...
	public function GetAnotherPersonListByUserId($user_id) {
		...
		*/
		$SQLQuery = "SELECT
						a.id, b.first_name, b.LastName, s.user_id as subscribe
					FROM user a LEFT JOIN profiles b ON b.user_id = a.id
					JOIN (SELECT user_id FROM subscription_user WHERE status = 5) s ON s.user_id = a.id
					WHERE (a.id = :UserId or parent_user_id = :UserId_One) AND a.Status = 1
					GROUP BY a.id
					ORDER BY a.id";
	...
	public function GetPersonDetailByPersonId($person_id) {
		/* $SQLQuery = "SELECT 
							a.id, a.UserName, b.UserProfileId, b.first_name, b.LastName, b.Gender, c.UserBirthDetailId, c.Day, c.Month, c.Year, c.Hours, c.Minutes, c.Seconds, c.unTimed, c.GMT, c.ZoneRef, c.SummerTimeZoneRef, c.Longitute, c.Lagitute, c.CreatedDate, c.ModifiedDate, c.CreatedBy, c.ModifiedBy, c.country, c.state, c.city, c.country_name, c.sunsign
					FROM `user` a
					LEFT JOIN `profiles` b ON b.user_id = a.id
					LEFT JOIN `birth_details` c ON c.user_id = a.id
			WHERE a.id = :UserId "; */
			 $SQLQuery = "SELECT 
							a.id, a.UserName, b.UserProfileId, b.first_name, b.LastName, b.Gender, c.Day, c.Month, c.Year, c.Hours, c.Minutes, c.Seconds, c.unTimed, c.GMT, c.ZoneRef, c.SummerTimeZoneRef, c.Longitute, c.Lagitute, c.CreatedDate, c.ModifiedDate, c.CreatedBy, c.ModifiedBy, c.country, c.state, c.city, c.country_name, c.sunsign
					FROM `user` a
					LEFT JOIN `profiles` b ON b.user_id = a.id
					LEFT JOIN `birth_details` c ON c.user_id = a.id
			WHERE a.id = :UserId "; 
	...
	public function GetUserDetailByUserId($user_id) {
		$SQLQuery = "SELECT
						u.id, u.UserName, u.Status, u.DefaultLanguage, u.PortalId, u.UserGroupId, up.first_name, up.LastName, up.Gender
					FROM `user` u
	...
	{
		$SQLQuery = "SELECT o.optin_option_id, o.user_id FROM `user_optin` o LEFT JOIN user u on u.id = o.user_id
	...
	{
		$SQLQuery = "UPDATE `user` SET Status = 0 WHERE id = :user_id";
	...
	{
		$SQLQuery = "SELECT 
						u.id, u.Password, u.DefaultLanguage , up.first_name, up.LastName 
						FROM `user` u LEFT JOIN `profiles` up ON u.id = up.UserId 
	...
	public function UpdateUserGroup($UserId, $EMail, $GroupId) {
		...
		$SQLQuery = "UPDATE `user` SET UserGroupId = :UserGroupId WHERE id = :UserId";
	...
	public function GetUserIds($UserId){
		/*$SQLQuery = "SELECT u.id, UserBirthDetailId, UserProfileId
				FROM `user` u, `birth_details` ubd, `profiles` up
				WHERE u.id = ubd.user_id AND up.user_id = ubd.user_id AND u.id = up.user_id AND u.id = :UserId ";*/
		$SQLQuery = "SELECT u.id, UserProfileId
				FROM `user` u, `birth_details` ubd, `profiles` up
				WHERE u.id = ubd.user_id AND up.user_id = ubd.user_id AND u.id = up.user_id AND u.id = :UserId ";
	...
	public function GetUserDetailByUserIdForSignUp($user_id) {
		...
		$SQLQuery = "SELECT 
						u.id, u.UserName, u.Status, u.DefaultLanguage, u.PortalId, u.UserGroupId, up.first_name, up.LastName, up.Gender 
						FROM `user` u LEFT JOIN `profiles` up on up.user_id = u.id 
						WHERE u.UserGroupId = 7 AND u.id = :UserId";
	...
	public function SaveUserBirthDetailStep1($data) {
		...
		$UserBirthDetailId = 0;
		$SQLQuery = "INSERT INTO `userbirthdetail`
						(user_id, Day, Month, Year, Hours, Minutes, Seconds, unTimed, GMT, ZoneRef, SummerTimeZoneRef, Longitute, Lagitute, 
	...
	public function GetSubScribeListByUserId($user_id) {
		$SQLQuery = "SELECT a.id, b.first_name, b.LastName ,
						(SELECT user_id FROM subscription_user WHERE user_id = a.id AND status = 5 GROUP BY user_id) as subscribe
					FROM user a left join profiles b on b.user_id = a.id
					WHERE (a.id = :UserId OR parent_user_id = :UserId_One
						OR a.id IN (SELECT user_id FROM subscription_user WHERE user_id = :UserId_Two AND status IN (1, 4, 0)
					GROUP BY user_id) )
					ORDER BY a.id ";
