...
// 		$this->SetX($col);
		...
		$this->col = $col;
		$x = 20 + $col * $this->column_width;
		
		//echo "SetCOL :: x = $x => Y = ". $this->GetY()." <br />";
	...
	function AcceptPageBreak() {
		if($this->col < 1) {			 // i.e. = 0
			...
			$this->SetCol(0);
			$this->SetLeftMargin(20); 		// was 10 but need a 6mm gutter
			//$this->SetY(62);
...
// 			echo "</pre>";
	...
	function CreateIndex() {
	    ...
	    $this->MultiCell(180, 5,
	    		$GenericText[$Global_Language]['Annotations'],
	    		/* no border */			0,
	    		/* align justified */	'C',
	    		/* transparent fill */ 	0);
	    $this->SetFontSize(10);
	    ...
	    for ($i=0;$i<$size;$i++){
	        ...
	        $NewPage = intval($NewPage ) + $FirstSectionPageNo;
	        //$this->Cell($PageCellSize,$this->FontSize+2,'p. '.$NewPage,0,1,'R');
	        
	        //$FinalContent = $str.$dots.' p. '.$NewPage;
			$FinalContent = $str.$dots;
			$this->Cell(190, $this->FontSize+2,$FinalContent, 0, 1, 'L'); 
