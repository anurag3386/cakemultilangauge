...
class GenericHouseAndSignFinder {
	...
	//private $ColumnWidth = 180; //One Column
	
	private $ColumnWidth = 83; //One Column
	...
	function PrintIntroctionTitle($PDFClass, $Content) {
		if($Content != "") {
			...
			//echo "PrintIntroctionTitle : XY :: " . $PDFClass->GetY() . "==" . $PDFClass->GetX() . "<br />";
			//$PDFClass->SetY(50);
			$StrLen = intval( $PDFClass->GetStringWidth($Content) ) + 10;
			$PDFClass->MultiCell($this->ColumnWidth, 5, trim($Content),  /* no border */			0, /* align justified */	'C', /* transparent fill */ 	0);
			$PDFClass->Ln();			 
		}
	}	

	function FirstPagePrintIntroctionTitle($PDFClass, $Content) {
		if($Content != "") {
			$PDFClass->SetFont( 'arial', 'B', 17);
			//$PDFClass->SetTextColor( $GLOBALS['SectionHeadingColor'][0], $GLOBALS['SectionHeadingColor'][1], $GLOBALS['SectionHeadingColor'][2]);
			$PDFClass->SetFillColor( 255, 255, 255 );
	...
	function PrintChapterHeader($PDFClass, $Content) {
		if($Content != "") {
			...
			//$PDFClass->SetTextColor( $GLOBALS['SectionHeadingColor'][0], $GLOBALS['SectionHeadingColor'][1], $GLOBALS['SectionHeadingColor'][2]);
			$PDFClass->SetTextColor( 0, 0, 0);
			$PDFClass->SetFillColor( 232, 232, 232 );  //255, 255, 255
			
// 			echo "--Bookmark() :: $Content <br />"; 
// 			$PDFClass->Bookmark($Content, 0, -1);
			
			//Following line is commented due to set up the Bookmarks
			$StrLen = intval( $PDFClass->GetStringWidth($Content) ) + 10;		
			//$PDFClass->MultiCell(180, 7, $Content,  /* no border */			0, /* align justified */	'L', /* transparent fill */ 	0);
			//Above line is commented due to set up the Bookmarks
			
			//Add For two columns layout
			$PDFClass->MultiCell($this->ColumnWidth, 10, strtoupper( trim($Content) ),  /* no border */			1, /* align justified */	'C', /* transparent fill */ 	1);
	...
	function PrintSectionInternalTitle($PDFClass, $Content) {
		...
		if($Content != "") {
			//echo "XY :: " . $PDFClass->GetY() . "==" . $PDFClass->GetX() . "<br />";
			...
			//echo "--PrintSectionInternalTitle() == Bookmark() :: $Content <br />";
			//$PDFClass->Bookmark($Content, 1, -1);
	...
	function PrintSectionDescription($PDFClass, $Content, $AnnotationNo = '', $Link = '') {	 
		...
		if($Content != "") {
			//echo "PrintSectionDescription XY :: " . $PDFClass->GetY() . "==" . $PDFClass->GetX() . "<br />";
...
// 			$PDFClass->SetRightMargin($RightMargin);
	...
	function FirstPagePrintSectionDescription($PDFClass, $Content, $AnnotationNo = '', $Link = '') {
		...
		if($Content != "") {
			...
			$PDFClass->MultiCell(180, 5, trim($Content),  /* no border */			0, /* align justified */	'L', /* transparent fill */ 	0);
