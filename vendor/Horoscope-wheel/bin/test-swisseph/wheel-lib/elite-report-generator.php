...
// 				break;
	...
	function SetIntroductionText() {
		...
		foreach($IntroductionText[$Global_Language] as $Key => $Value) {
			if($Key > 0) {
				$GenericHelper->FirstPagePrintSectionDescription($this, $Value);
			}
		}		
...
// 		$this->SetX(20);
		...
		$GenericHelper->FirstPagePrintIntroctionTitle($this, $IntroductionText[$Global_Language]["Introduction"]);	//For 1st page of the Year Introduction
...
// 		}
		...
		$FirstSectionPageNo = $this->PageNo();			
...
// 			$this->GenericHelper->PrintSectionInternalTitle($this, $AgeTitle);
			$this->GenericHelper->FirstPagePrintSectionDescription($this, $AgeDescription);
		}
	}
	...
	function GenericGrowthText(){
		...
		$GenericTextGetter->FirstPagePrintSectionDescription($this, $GenericText[$Global_Language]['Growth_text']);
	}
	...
	function PrintThemeText() {
		...
		//Theme Tester
		echo "<pre>$FinalText </pre>"; 
		$GenericTextGetter->FirstPagePrintSectionDescription($this, $FinalText);
	}
	...
	function PrintThemeTester() {
		...
		foreach ($this->TopTheme as $key => $trWin) {
			...
			if($TesterText != "") {
				...
				//Theme Tester
				///echo "<pre>".sprintf("- %s",  $TesterText)."</pre>";
	...
	function GenericTheme2text(){
		echo "<pre>********************************************************************** GenericTheme2text()</pre>";
		global $Global_Language;
		global $GenericText;
		$GenericClass = new GenericHouseAndSignFinder();

		//$GenericClass->PrintSectionInternalTitle($this, '#3THEMES_2');
		//echo "<pre>". $GenericText[$Global_Language]['3THEMES_2'] ."</pre>";
		$GenericClass->FirstPagePrintSectionDescription($this, $GenericText[$Global_Language]['3THEMES_2']);
	}
...
// 		$this->AddPage();
	...
	function ValuesANDLoveANDRelationships() {
		...
		//Commentted Due to two column layout
// 		$this->SetMyMarging();
// 		$this->AddPage();
	...
	function SectionHeadlines($GenericHelper, $PrintSectionFor, $SectionNo) {
		...
		foreach ($this->SectionHeadingContents as $key => $trWin) {
			...
			if($PrintSectionFor == $pt)	{
				...
				array_push($TableOfContent, array("ChapterName" => $MainHeading, "PageNo" => $this->PageNo()));

				//$this->Bookmark($Maincontent, 0, -1);
		...
		function SetGenericProgressedText($ChapterNo, $PlanetCode1, $PlanetCode2, $AspectType, $AspectRadixHitDate, $AspectPRHitDate = '', $FetchLastText = 'N', $FetchNextText = 'N', $PassedConnector = '+') {
			...
			if(count($ReturnArray) > 0 && $ReturnArray[0]['description'] != '') {
				...
				if($TransitTitle != '') {
					...
					$this->SetTextColor( $GLOBALS['ContentColor'][0], $GLOBALS['ContentColor'][1], $GLOBALS['ContentColor'][2]);
					$this->SetFillColor( 0xcc, 0xcc, 0xcc );
					$this->GenericHelper->PrintSectionInternalTitle($this, $TransitTitle);
	...
	private function AddWheelPage() {
		...
		if (file_exists($ImportFileName)) {
			...
			for ($currentIndex = 1; $currentIndex <= $TotalPages; $currentIndex++) {
				...
				$this->useTemplate($currentImportedPage, 0, 0, 210, 297);
		
				//Left, Top, [Right] Just because in safe side
				$this->SetMargins(0.50, 0.50);
			}
		}
	}
}
