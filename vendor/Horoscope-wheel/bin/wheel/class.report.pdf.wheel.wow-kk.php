...
class PDF_Wheel_WOW extends PDF_Wheel {
	...
	function ApplyCustomisations() {
		...
		$this->glyph_font_family = 'arial'; //'wows';
...
//		$this->Image(RESOURCE_PATH.'images/ppt_glass_design.jpg',-1,-1);
	...
	function generateChartWheelPage() {
		$this->language = (isset($_SESSION['locale']) && !empty($_SESSION['locale'])) ? $_SESSION['locale'] : 'en';
		if ($this->language == 'da') {
			$this->language = 'dk';
		}
		$this->AddPage();
		...
		if( $_SERVER['REMOTE_ADDR'] == '103.254.97.14' || $_SERVER['REMOTE_ADDR'] == '103.248.117.12' ) {
			$this->drawWheelPlanetskk();
		} else {
			$this->drawWheelPlanets();
		}
		$this->drawWheelAspects();
		$this->drawTableUserInfo( $this->language ); // class.report.pdf.wheel.rc1.php
		$this->drawTableSymbolInfo( $this->language ); // class.report.pdf.wheel.rc1.php
