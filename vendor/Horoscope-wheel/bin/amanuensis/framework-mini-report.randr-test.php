...
// 	) );
	...
	$orderqueue = $order->GetList ( array( array ('order_id', '=', 65681 ) ) ); 		/** AMIT - SEASONAL */
} else {	
	$orderqueue = $order->GetList ( array(
			array ('order_status', '=', 12 ),                                       	/* Queued */ 
			array ('portalid', '=', intval ( $affiliate_id ) ),                     	/* Portal ID */
	...
	$orderqueue1 = $order->GetList ( array( array ('order_id', '=', 64897 ) ) ); /** SEASONAL */
...
if (count ( $orderqueue ) > 0) {
	...
	foreach ( $orderqueue as $orderitem ) {
		...
		*/
		...
		if( $orderitem->email_id == "amit@test") {
