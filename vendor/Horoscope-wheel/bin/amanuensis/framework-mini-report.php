...
require_once (CLASSMINIREPORTPATH . '/report-generator/PDF.Report.Generator.WOW.php');
...
if(strpos(ROOTPATH, "public_html")!== FALSE) {
	$orderqueue = $order->GetList ( array(
			...
			array ('product_item_id', 'IN', "(23, 25)"),                             	/* Personal - 23 / Seasonal - 24 */
	...
	$orderqueue = $order->GetList ( array(
			...
			array ('product_item_id', 'IN', "(23, 24)"),                             	/* Personal / Seasonal */
			array ('language_code', '=', $ReportLanguage),                         		/* REPORT Language*/
	) );
	
	$orderqueue = $order->GetList ( array( array ('order_id', '=', 38892 ) ) );  /** PERSONAL */
	//$orderqueue1 = $order->GetList ( array( array ('order_id', '=', 64896 ) ) ); /** PERSONAL */
	$orderqueue1 = $order->GetList ( array( array ('order_id', '=', 38893 ) ) ); /** SEASONAL */

	$orderqueue = array_merge($orderqueue, $orderqueue1);
		...
		*/
		...
		switch ($orderitem->product_item_id) {
			...
			case 35 :                       //Seasonal Report for 3 Months with Questions and Anwser
				$ProductName = "Seasonal mini reading";
		...
		echo "*************************************<br />";
		$tcpdf = new WOW_TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);		
		// set some language-dependent strings (optional)
		
		$tcpdf->setLanguageArray($l);
		$tcpdf->SetReportLanguage($ReportsDefaultLanguage);
		echo $ReportType ." ReportType<br />";
		$tcpdf->SetReportType($ReportType);
		$tcpdf->SetFranchise("AstroWOW", "Astrowow.com");
		$tcpdf->SetCoverPage();
