...
$SQLQuery .= " WHERE (mp.portalid > 148 AND  mp.portalid <> 166 ) AND mp.userid = mp.userid AND mo.order_status IN (9,12) AND membership_enddate >= :MembershipEndDate ";
$SQLQuery .= " GROUP BY  mp.userid, mp.portalid ";
...
echo "[AMIT] - [".count($eLiteActiveRow)."]";

if(count($eLiteActiveRow) > 0) {
	...
	foreach ($eLiteActiveRow as $CurrentRow) {
		...
		if( count($orderqueue) > 0 ) {
			...
			$OrderString = implode(",", $ordid);
		
			if($OrderString != "") {
				$SQLQuery = "UPDATE `order` SET order_status = 4 WHERE order_id IN ($OrderString) ";
				$objMyPDO = $db->prepare($SQLQuery);
				$objMyPDO->execute();
			}
		}
		
		echo "[-AMIT-] - [".$CurrentRow['userid']."]-[".$CurrentRow['portalid']."]\\n";
		...
		if (count ( $orderqueue ) > 0) {
			...
			*/
		    ...
		    foreach ( $orderqueue as $orderitem ) {
		        ...
		        echo "<pre>[==ORDER STATUE ::". $orderitem->order_status.  " == ]</pre>";
		        ...
		        foreach ( $birthdatalist as $birthentry ) {            
		            ...
		            $timed_data = ((trim ( $birthentry->hour ) == '-1') ? false : true);
		...
		//                $start_year = intval ( substr ( $birthentry->start_date, 0, 4 ) );
		        ...
		        */
		        ...
		        $wheel->table_user_info_birth_state = $birthcountry;
				...
				//Showing Sun, moon and Accendant in last column [07-Jun-2014]
				...
				echo "table_user_info_birth_weekday : " .$wheel->table_user_info_birth_weekday ."<br />";
				echo "table_user_info_house_system : " .$wheel->table_user_info_house_system ."<br />";
				echo "table_user_info_orb_system : " .$wheel->table_user_info_orb_system ."<br />";
...
// 		        		$pdi = new WOW_Col8_FPDI_EN ();
		        ...
		        switch ( $orderitem->product_item_id ) {
		            case 13 :   // PERSONAL REPORT
		                ...
		                $pdi->cover_heading = utf8_decode("Personal Report for...");
		                break;
		        ...
		        if ($orderitem->delivery_option == 1) {                 /* email */
		            ...
		            switch (strtoupper ( $ReportsDefaultLanguage )) {
		            	...
		            	case 'EN' :
		                    ...
		                    $email = new WoweLiteCustomerEmailDelivery_EN ();
		                    break;
		            	case 'SWEDISH' :
		            	case 'SE' :
		            ...
		            //FOR PREVIEW REPORT
		            $email->ReportName = $title;
		            $email->CustomerName = $birthentry->first_name;
