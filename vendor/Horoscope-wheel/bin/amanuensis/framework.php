        ...
        switch ($orderitem->portalid) {
            ...
            case C_AFFILIATE_WOW :
                switch (strtoupper ( $ReportsDefaultLanguage )) {
            			...
            			case 'DE' :
            				//German Language
            				require_once (CLASSPATH . '/collate/class.collate.wow.de.php');
            				$pdi = new WOW_Col8_FPDI_DK ();
            				break;
            			case 'DANISH' :
            			case 'DK' :
            				require_once (CLASSPATH . '/collate/class.collate.wow.dk.php');
            				$pdi = new WOW_Col8_FPDI_DK ();
            				break;
            			default:
            				$pdi = new WOW_Col8_FPDI_EN ();
...
//                $start_year = intval ( substr ( $birthentry->start_date, 0, 4 ) );
        ...
        /** Newly added function to insert new update Cover Page [07-Dec-2015] **/
        $pdi->SetupCoverPage();
        /** Newly added function to insert new update Cover Page [07-Dec-2015] **/
        ...
        DeleteHI_AC_UnwantedFiles(SPOOLPATH, $orderitem->order_id);
...
function DeleteHI_AC_UnwantedFiles($SPOOLPATH, $OrderNumber) {
	#complete serverpath must be given like
	#example "/apache/htdocs/myfile.pdf" ( not "http:xyz.com/myfile.pdf" )

	# delete file if exists
	$DeleteReportPDF = sprintf("%s/%d.report.pdf", $SPOOLPATH, $OrderNumber);
	$DeleteWheelPDF = sprintf("%s/%d.wheel.pdf", $SPOOLPATH, $OrderNumber);
	$DeletePreviewBundlePDF = sprintf("%s/%d.preview_bundle.pdf", $SPOOLPATH, $OrderNumber);

	if (file_exists($DeleteReportPDF))   {
		@unlink ($DeleteReportPDF);
	}
	if (file_exists($DeleteWheelPDF))   {
		@unlink ($DeleteWheelPDF);
	}
	if (file_exists($DeletePreviewBundlePDF))   {
		@unlink ($DeletePreviewBundlePDF);
	}
}
?>