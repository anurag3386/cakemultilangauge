...
require_once (CLASSPATH . '/wow/bb.forecast.php');
require_once (CLASSPATH . '/wow/bb.dynamicanalysis.php');
...
require_once (ROOTPATH . '/bin/wheel/class.report.pdf.whee.bb.php');
...
require_once (CLASSPATH . '/postback/class.report.postback.notification.php');
require_once (CLASSPATH . '/awswrapper/class.asw.s3.wrapper.php');
...
$SQLQuery .= " WHERE mp.portalid = 148 AND mp.userid = mo.user_id AND mo.order_status IN (9,12) ";
...
echo "\n\n SQLQuery::  $SQLQuery \n\n ";
echo "\n\n RCount " . count($eLiteActiveRow). "\n\n";
if($eLiteActiveRow) {
	
	foreach ($eLiteActiveRow as $CurrentRow) {
		
		/* look for queued orders */
		$logger->debug ( "Amanuensis::Framework-BermanBraun - look for queued orders" );
		$order = new Order ();
		...
		if (count ( $orderqueue ) > 0) {
		    ...
		    $logger->debug ( 'Amanuensis::Framework-BermanBraun - order queue has ' . count ( $orderqueue ) . ' orders' );
		    ...
		    foreach ( $orderqueue as $orderitem ) {
		        $logger->debug ( 'Amanuensis::Framework-BermanBraun - starting to process order' );
		        ...
		        $logger->debug ( 'Amanuensis::Framework-BermanBraun - setting order state - processing' );
		        $orderitem->order_status = $states ['processing'];
		        $orderitem->save ();
		        echo "\n\n[==ORDER-NO::". $orderitem->order_id."==]\n\n";
		        		        
		        /* update transactions */
		        $logger->debug ( 'Amanuensis::Framework-BermanBraun - creating new transaction - processing' );
		        ...
		        $logger->debug ( 'Amanuensis::Framework-BermanBraun - processing birthdata' );
		        ...
		        if (count ( $birthdatalist ) != 1) {
		            $logger->debug ( 'Amanuensis::Framework-BermanBraun - multiple or no birthdata found' );
		        } else {
		            $logger->debug ( 'Amanuensis::Framework-BermanBraun - birthdata found' );
		        }
		
		        foreach ( $birthdatalist as $birthentry ) {
		            $formattedDate = sprintf ( "%04d%02d%02d", $birthentry->year, $birthentry->month, $birthentry->day );
		            $logger->debug ( 'Amanuensis::Framework-BermanBraun - formatted date = ' . $formattedDate );
		
		            $formattedTime = sprintf ( "%02d:%02d", $birthentry->hour, $birthentry->minute );
		            $logger->debug ( 'Amanuensis::Framework-BermanBraun - formatted time = ' . $formattedTime );
		
		            /**
		             * added for solar chart
		             */
		            $timed_data = ((trim ( $birthentry->untimed ) == '1') ? false : true);
		            $logger->debug ( "Amanuensis::Framework-BermanBraun - timed data = [" . trim ( $birthentry->untimed ) . "]" );
...
// 		            echo "<pre> CalLat : $CalLat </pre>";
		        ...
		        $logger->debug ( 'Amanuensis::Framework-BermanBraun - processing report options' );
		        $logger->debug ( 'Amanuensis::Framework-BermanBraun - creating PDF generator for default case ( A4 size )' );
		        $generator = new PDFGenerator ( 'A4' );
		        
		        $generator->language = $ReportsDefaultLanguage;
		        $logger->debug ( 'Amanuensis::Framework-BermanBraun - generator language set to ' . $ReportsDefaultLanguage );
		        switch (strtoupper ( $generator->language )) {
		            case 'ENGLISH' :
		            case 'EN' :
		                $logger->debug ( 'Amanuensis::Framework-BermanBraun - generator using English Book' );
		                $book = new BookUK ();
		                $shorttermtrend = new ShortTermTrend ();
		                break;
		            case 'DANISH' :
		            case 'DK' :
		                $logger->debug ( 'Amanuensis::Framework-BermanBraun - generator using Danish Book' );
		                $book = new BookDK ();
		                $shorttermtrend = new ShortTermTrend_DK ();
		                break;
		            case 'DUTCH' :
		            case 'DU' :
		                $logger->debug ( 'Amanuensis::Framework-BermanBraun - generator using Dutch Book' );
		                $book = new BookDU ();
		                $shorttermtrend = new ShortTermTrend (); /* use UK trends until translation exists */
		                break;
		            case 'DE' :
		                $logger->debug ( 'Amanuensis::Framework-BermanBraun - generator using German Book' );
		                $book = new BookGE ();
		                $shorttermtrend = new ShortTermTrend (); /* use UK trends until translation exists */
		                break;
		            case 'GREEK' :
		            case 'GR' :
		                $logger->debug ( 'Amanuensis::Framework-BermanBraun - generator using Greek Book' );
		                $book = new BookGR ();
		                $shorttermtrend = new ShortTermTrend (); /* use UK trends until translation exists */
		                break;
		            case 'NO' :
		                $logger->debug ( 'Amanuensis::Framework-BermanBraun - generator using Norwegian Book' );
		                $book = new BookNO ();
		                $shorttermtrend = new ShortTermTrend (); /* use UK trends until translation exists */
		                break;
		            case 'SPANISH' :
		            case 'SP' :
		                $logger->debug ( 'Amanuensis::Framework-BermanBraun - generator using Spanish Book' );
		                $book = new BookSP ();
		                $shorttermtrend = new ShortTermTrend (); /* use UK trends until translation exists */
		                break;
		            case 'SWEDISH' :
		            case 'SE' :
		                $logger->debug ( 'Amanuensis::Framework-BermanBraun - generator using Swedish Book' );
		                $book = new BookSW ();
		                $shorttermtrend = new ShortTermTrend (); /* use UK trends until translation exists */
		                break;
		            default :
		                ...
		                /* update state */
		                $logger->debug ( 'Amanuensis::Framework-BermanBraun - setting order state - orphaned' );
		                $orderitem->order_status = $states ['orphaned'];
		                $orderitem->save ();
		
		                /* update transactions */
		                $logger->debug ( 'Amanuensis::Framework-BermanBraun - creating new transaction - orphaned' );
		                $transaction = new Transaction ();
		                $transaction->orderId = $orderitem->order_id;
		                $transaction->state = $orderitem->order_status;
		                $transaction->timestamp = strftime ( "%Y-%m-%d %H:%M:%S" );
		        ...
		        */
		        ...
		        switch ($orderitem->product_item_id) {
		            case 13 :                                   //Personal Report
		                ...
		                $report->type = 'pc3';
		                $logger->debug ( 'Amanuensis::Framework-BermanBraun - report type = ' . $report->type );
		                ...
		                $logger->debug ( 'Amanuensis::Framework-BermanBraun -ANP $start_year - $start_month = ' . $start_year . ' - ' . $start_month );
		                if ($start_month == 11) {
		                    $start_month = 1;
		                    $start_year ++;
		                }
		                ...
		                $logger->debug ( 'Amanuensis::Framework-BermanBraun -ANP $start_date = ' . $start_date );
		                $analysis_context = new BB_DynamicAnalysis(
		            ...
		            case 17 :                       //Seasonal Report for 3 Months with Questions and Anwser
		                ...
		                $report = new BB_Forecast ();
		                $generator = new PDF_Forecast ();
		                $logger->debug ( "Amanuensis::Framework-BermanBraun - generator for seasonal report" );                
		        ...
		        */
		        $logger->debug ( 'Amanuensis::Framework-BermanBraun - processing wheel' );
		        //$wheel = new PDF_Wheel_WOW ();
		        $wheel = new PDF_Wheel_BB ();
		        ...
		        if ($timed_data === true) {
		        	...
		        	$birthdate = sprintf ( "%02d/%02d/%04d %02d:%02d", $birthentry->month, $birthentry->day, $birthentry->year, $birthentry->hour, $birthentry->minute );
		        } else {
		            //$birthdate = sprintf ( "%02d/%02d/%04d", $birthentry->day, $birthentry->month, $birthentry->year );
		            $birthdate = sprintf ( "%02d/%02d/%04d", $birthentry->month, $birthentry->day, $birthentry->year );
		        }
		        ...
		        $wheel->table_user_info_birth_place = str_replace("Not Found:", "", trim ( $birthplace )) ;
		        //$wheel->table_user_info_birth_place = trim ( $birthentry->place );
		        $wheel->table_user_info_birth_state = str_replace("Not Found:", "", $FullPlaceName);
		         ...
		         * *********************************************
		        */
		        $logger->debug ( 'Amanuensis::Framework-BermanBraun - processing assembly' );
		        $generator->language = $ReportsDefaultLanguage;
		        
		        
		        /**
		         * @todo: Right now we are providing only English version reports to BermanBraun, In future if they requires 
		         *        multiple language, we just new to create following classes to provide multiple language support.
		         *        -WOW_eLite_Col8_FPDI_SE to BermanBraun_Col8_FPDI_SE
		         *        -WOW_eLite_Col8_FPDI_DE to BermanBraun_Col8_FPDI_DE
		         *        -WOW_eLite_Col8_FPDI_DK to BermanBraun_Col8_FPDI_DK        
		         */
		        switch (strtoupper ( $ReportsDefaultLanguage )) {
		        	case 'ENGLISH' :
		        	case 'EN' :
		        		require_once (CLASSPATH . '/collate/class.collate.bermanbraun.en.php');
		        		$pdi = new BermanBraun_Col8_FPDI_EN ();
		        		break;
		        	case 'SWEDISH' :
		        	case 'SE' :
		        ...
		        switch ( $orderitem->product_item_id ) {
		            case 13 :   // PERSONAL REPORT
		                ...
		                $date_start = sprintf ( '%s 01, %04d', $MonthArray[intval( $start_month)], $start_year );
		                $date_end = sprintf ( '%s 01, %04d', $MonthArray[intval( $start_month)], ($start_year + $birthentry->duration) );
		                
		                $title = "My Life Path Report & Birth Chart ($date_start - $date_end)";		                
		                $pdi->table_of_contents = $report->generate->toc;
		                
		                $pdi->cover_heading = utf8_decode("My Life Path Report and Birth Chart");		                
		            ...
		            case 17 :   // SEASONAL REPORT
		                ...
		                switch (strtoupper($ReportsDefaultLanguage)) {
		                    case 'EN':
		                        $title = "Personal 3-Month Day-To-Day Planner";
		                        $pdi->cover_heading = utf8_decode("3-Month Day-To-Day Planner for");
		                        break;
		                    case 'DK':
		        ...
		        /* update state */
		        $logger->debug ( 'Amanuensis::Framework-BermanBraun - setting order state - ready' );
		        $orderitem->order_status = $states ['ready'];
		        $orderitem->save ();
		
		        /* update transaction */
		        $logger->debug ( sprintf ( "Amanuensis::Framework-BermanBraun - creating transaction, order state = %d", $orderitem->order_status ) );
		        $transaction->state = $orderitem->order_status;
		        $logger->debug ( sprintf ( "Amanuensis::Framework-BermanBraun - creating transaction, timestamp = %s", strftime ( "%Y-%m-%d %H:%M:%S" ) ) );
		        $transaction->timestamp = strftime ( "%Y-%m-%d %H:%M:%S" );
		        $logger->debug ( 'Amanuensis::Framework-BermanBraun - creating transaction, savenew' );
		        $transaction->savenew ();
		        $logger->debug ( 'Amanuensis::Framework-BermanBraun - creating transaction, done' );
		                
		        /* if delivery method is email then deliver the report */
		        $logger->debug ( 'Amanuensis::Framework-BermanBraun - checking delivery option' );
		
		        echo " [ === Delivery Option : " .$orderitem->delivery_option. '=== ]\\n<br />  ';
		        
		        /**
		         * @todo: Here I need to inject the code for FTP uploads and push the notification to BermanBraun
		         */		
		        if ($orderitem->delivery_option == 1) {                 /* email */
		        	
		        	try {
		        		$aws = new ASWUploadWrapper($orderitem->order_id, sprintf ( "%s/%d.bundle.pdf", SPOOLPATH, $orderitem->order_id ) );
		        		$aws->OrderId = $orderitem->order_id;
		        		$aws->UploadNow();
		        	}
		        	catch(Exception $ex) {
		        		echo "\n\n There is Error While Uploading to AWS : $ex->getMessage() \n\n ";
		        	}
		        	echo "\n\n AWS Upload Done\n\n";		        	
		        	
		        	try {		        			
		        		$rptNotification = new ReportNotification();
		        		$rptNotification->OrderId = $orderitem->order_id;
		        		$rptNotification->StatusCode = 5;		//SEASONAL = 1 || YEAR REPORT = 2
		        		$rptNotification->SendNotification();
		        	}
		        	catch(Exception $ex) {
		        		echo "\n\n OrderID :".$orderitem->order_id ." =There is Notification Error : $ex->getMessage() \n\n ";
						$orderitem->order_status = $states ['closed'];
						$orderitem->save ();
		        	}
		        	echo "\n\n Postback script Done \n\n ";
		        	
		        	$orderitem->order_status = $states ['closed'];
		        	$orderitem->save ();
		        	
		            $logger->debug ( 'Amanuensis::Framework-BermanBraun - processing mail delivery' );
		            echo " [ === Portal Id : " .$orderitem->portalid. '=== ]\\n<br />  ';
		            /*
		            switch (strtoupper ( $ReportsDefaultLanguage )) {
		            	case 'ENGLISH' :
		            	case 'EN' :
		            		$logger->debug ( 'Amanuensis::Framework-BermanBraun - processing mail delivery for WOW(UK)' );
		                    $email = new WowEmailDelivery_EN ();
		                    break;
		            	case 'SWEDISH' :
		            	case 'SE' :
		            		$logger->debug ( 'Amanuensis::Framework-BermanBraun - processing mail delivery for WOW(SE)' );
		                    $email = new WowEmailDelivery_SE ();
		                    break;
		            	case 'DE' :
		            		//German Language
		            		$logger->debug ( 'Amanuensis::Framework-BermanBraun - processing mail delivery for WOW(DK)' );
		            ...
		            */
		            ...
		            $logger->debug ( 'Amanuensis::Framework-BermanBraun - no processing mail delivery required' );
		    ...
		    // $logger->debug('Amanuensis::Framework-BermanBraun - order queue is empty');
		}
	}
}
