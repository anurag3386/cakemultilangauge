...
if(strpos(ROOTPATH, "public_html")!== FALSE) {
	$orderQueueList = $orderDAL->GetList ( array(
			...
			array ('product_item_id', '=', "24"),                             		/* YEAR REPORT */
	) );
	echo "affiliate_id -- $affiliate_id -\\n\\n"; 
...
//Setup Global Variables
echo "\n\n MINI-YEAR-REPORT-COUNT  " .count($orderQueueList) . " YEAR REPORT \n\n";
	...
	foreach ($orderQueueList as $orderitem) {
		...
		if(count($birthDataObj) > 0) {
			...
			**/
			...
			$emailObj = new WowMiniYREmailDelivery_EN ($reportLanguage);
			//$emailObj = new WowYearReportEmailDelivery_EN();
						
			$emailObj->CustomerName = $UserFName;
			$emailObj->ReportName = "Mini Yearly reading.";
