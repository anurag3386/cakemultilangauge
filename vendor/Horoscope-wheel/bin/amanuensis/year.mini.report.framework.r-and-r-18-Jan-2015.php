...
// 			$Global_CurrntYear= date("Y-m-d", mktime ( 0, 0, 0, $bData->month, $bData->day, date('Y', strtotime( $bData->start_date ) ) ) );
			...
			$reportGenerator->Output(sprintf("%s/%d.report.pdf", SPOOLPATH, $orderitem->order_id));
			$reportGenerator->Close();
			unset($reportGenerator);
			
			$pdi = new MINI_RandR_Col8_FPDI_EN('year');			
			$pdi->introduction_context = 'year';
			/* footer stuff */
...
function DeleteUnwantedFile($SPOOLPATH, $OrderNumber) {
	...
	$DeleteTransitgraphFile = sprintf("%s/%d.report.pdf", $SPOOLPATH, $OrderNumber);
