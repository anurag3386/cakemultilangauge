...
require_once(ROOTPATH . '/bin/test-swisseph/report-pdf/bermanbraun.finishing.pdf.php');
//require_once(ROOTPATH . '/bin/test-swisseph/report-pdf/commonpdfhelper.php');
require_once(ROOTPATH . '/bin/test-swisseph/report-pdf/bermanbraun.commonpdfhelper.php');
require_once(ROOTPATH . '/bin/test-swisseph/wheel-lib/generate-wheel-pdf-bb.php');
...
require_once(ROOTPATH . '/bin/test-swisseph/wheel-lib/bermanbraun-report-generator.php'); 	//ADDED ON 18-Jan-2014
...
require_once(ROOTPATH . '/bin/test-swisseph/report-pdf/bermanbraun.class.generichelper.php'); 	//ADDED ON 18-Jan-2014
...
$orderQueueList = $orderDAL->GetList ( array(
                        ...
                        array ('portalid', '=', 148 ),                         /* Portal ID */
                        array ('product_type', '=', 5),                                             /* Only Email Version */
                        array ('product_item_id', '=', "19"),                                       /* YEAR REPORT */
                    ) );

//$orderQueueList = $orderDAL->GetList ( array( array ('order_id', '=', 38661 ) ) );
...
if (count($orderQueueList) > 0) {
    ...
    foreach ($orderQueueList as $orderitem) {
        ...
        //Alway takes English
        require_once(ROOTPATH . '/include/lang/year-report/en.bb.php');
        $YBookText  = new year_book_en();
        $YBookAgeText = new year_book_en_agetext();
       
        echo "\n\n[==YEAR-ORDER-NO::". $orderitem->order_id."==]\n\n";
        ...
        if(count($birthDataObj) > 0) {
            ...
            $reportGenerator->CurrentOrderId = $orderitem->order_id;
            ...
            $finalBundlePDF = new BermanBraunFinalBundleGeneraterPDF();
...
//          Set Report to Ready status
			...
			if($orderitem->delivery_option == 1) {
				...
				try {
					...
					echo "\n\n There is Notification Error : $ex->getMessage() \n\n ";
				}
...
//                 $birthentry->hour, $birthentry->minute);
    	...
    	$birthdate = sprintf("%02d/%02d/%04d %02d:%02d", $birthentry->month, $birthentry->day, $birthentry->year,
    			$birthentry->hour, $birthentry->minute);
    } else {
    ...
    $wheel->table_user_info_birth_state			= str_replace("Not Found:", "", trim($birthcountry));
