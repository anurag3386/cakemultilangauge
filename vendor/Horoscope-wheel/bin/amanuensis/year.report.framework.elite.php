...
$orderQueueList = $orderDAL->GetList ( array(
                        ...
                        array ('portalid', '<>', 166 ),                         		   /* Portal ID */						
                        array ('product_type', '=', 5),                                             /* Only Email Version */
                        array ('product_item_id', '=', "19"),                                       /* YEAR REPORT */
                    ) );
//$orderQueueList = $orderDAL->GetList ( array( array ('order_id', '=', 33979 ) ) );


$OrderString = '';
if( count($orderQueueList) > 0 ) {
	$ordid = array();
	foreach ($orderQueueList as $value) {
...
if (count($orderQueueList) > 0) {
    ...
    foreach ($orderQueueList as $orderitem) {
        ...
        if(count($birthDataObj) > 0) {
            ...
            $finalBundlePDF = new EliteFinalBundleGeneraterPDF();
            $finalBundlePDF->AliasNbPages();
            $finalBundlePDF->FranchiseId = $orderitem->portalid;
            $finalBundlePDF->PortalUserId = $orderitem->user_id;
...
//          	Creating Email Objects
				...
				if($orderitem->portalid == 156)	{
					//$emailObj = new WoweLiteCustomerEmailDelivery_EN ();
					$emailObj = new WoweLiteCustomerPPEmailDelivery_EN ();

					//FOR PREVIEW REPORT
					$emailObj->ReportName = "Essential Year ahead prediction";
					$emailObj->CustomerName = $memberName;
				} else {
...
function CreateBirthDTO($bData) {
    ...
    echo "== $bTimeZone :: $FinalZone ==  $bSummerTimeZone";
