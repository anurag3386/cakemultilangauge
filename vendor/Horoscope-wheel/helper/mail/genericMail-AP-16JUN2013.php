...
require_once(ROOTPATH."bal/product.php");
...
include(ROOTPATH."/library/swift/swift_required.php");
...
class genericMail {
	...
	function SendSubscriptionBuyEmail($sender,$recipient, $data ) {
		//$str = '<img title="Astrowow.com" alt="Astrowow.com" src="http://astrowow.com/images/logo.png">';
		$str  = '';
		$str .='<p>Thank you for placing your order with ##SITENAME !</p> ';
		$str .='<p>This email is to confirm your recent order.</p> ';
		$str .='<p>Date ##ORDERDATE</p>';
		...
		$str .='<ul style="list-style-type:none"> ';
		$str .='<li>';
		//$str .= '<img title="Astrowow.com" alt="Astrowow.com" src="http://astrowow.world-of-wisdom.com/images/img-astrology-software.jpg">';
		$str .='1 &nbsp;';
		$str .='##PRODUCTNAME &nbsp;';
		$str .='##PRODUCTPRICE &nbsp;';
		$str .='</li>';
	...
	function SendResetPasswordLinkToUser($sender,$recipient,$data) {
		...
		if(count($result)>0) {
			$str = html_entity_decode( utf8_decode($result['content']));
			$subject  = stripslashes(html_entity_decode( utf8_decode($result['subject'])));			
		}
		else {			
			$str = $data['mailtext'].'<img title="Astrowow.com" alt="Astrowow.com" src="'.BASEURL.'images/logo.png">';
	...
	function SendPaymentAcceptMailForReport($sender,$recipient,$data) {
		...
		if(isset($data['language_id'])) {
			if($data['language_id'] == 'English') {
				$language_id = 'en';
			}
			else if($data['language_id'] == 'Danish') {
				$language_id = 'dk';
			}
			else {
				$language_id = 'en';
	...
	function SendThankSubscriptionEmail($recipient, $data) {
		...
		//$body .= 'Your payment received for Order no #'.$this->ipn_data['invoice'].'<br />';
		//$body .= "<br /> Order No: ".$data['invoice'];
		//$body .= "<br /> Transaction No: ".$data['txn_id'];
		//$body .= "<br /> Item Name: ".$data['item_name'];
		//$body .= "<br /> Amount: ".$data['payment_gross'];
		//$body .= "<br /> Thanks, <br /> Astrowow Team";
