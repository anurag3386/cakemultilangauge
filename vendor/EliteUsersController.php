    ...
    function thankYouForReportPurchase () {
        ...
        if ($user_id) {
            if (!empty($session->read('EliteReportOrder'))) {
                ...
                if ($ordersTable->save($entity)) {
                    $currencycode = $this->getCurrencyDetails ($entity->currency_id);
                    $birthData = $this->saveBirthDataAsOrder ($entity->id);
                    $currencycode = $currencycode['code'];
                    $order_id = $payerId = $this->request->query ('orderid');
                    $transctions = $this->saveTransactionData ($entity->id, $session->read('EliteReportOrder.BirthData.first_name'), $session->read('EliteReportOrder.BirthData.last_name'), $session->read('EliteReportOrder.UserDetail.email'), $entity->price, $currencycode, $payerId);
                    $this->loadModel('Currencies');
            ...
            if ($ordersTable->save($data)) {
              ...
              if (empty($existancy)) {
                ...
                if ($query) {  
                  ...
                  $updateEliteMember = $this->EliteMembers->updateAll (['start_date' => $start, 'end_date' => $end, 'currency' => $session->read('Elite Currency Code'), 'currency_code' => $ecurrency_code, 'amount' => $eamount], ['user_id' => $user_id, 'id' => $existancy->id]);
